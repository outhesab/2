...
function buildSaleRecord(
  ...
  return {
    ...
    productName:
      intent.items.length === 1 ? firstItem.productName : `${firstItem.productName} +${intent.items.length - 1}`,
...
export function completeSale(intent: SaleIntent, db: DB): IntentResult {
  ...
  const nowIso = intent.saleDate ? new Date(intent.saleDate).toISOString() : new Date().toISOString();
  const saleId = genId();
  ...
  const cariUpdate =
    intent.payment === 'cari' && intent.cariId ? [{ cariId: intent.cariId, balanceChange: total }] : undefined;
...
export function cancelSale(saleId: string, db: DB): IntentResult {
  ...
  const cariUpdate =
    sale.payment === 'cari' && sale.cariId ? [{ cariId: sale.cariId, balanceChange: -(sale.total || 0) }] : undefined;
...
export function returnSale(saleId: string, db: DB, qty?: number | Record<string, number>): IntentResult {
  ...
  const kasaEntry: CashTransaction | null =
    sale.payment !== 'cari'
      ? {
  ...
  const cariUpdate =
    sale.payment === 'cari' && sale.cariId ? [{ cariId: sale.cariId, balanceChange: -returnedTotal }] : undefined;
