...
export const TABS = [
  ...
  { id: 'receivables', label: 'Alacak Takip', icon: CreditCard, group: 'Finans' },
  ...
  { id: 'nexusAdmin', label: 'Nexus Admin', icon: Brain, group: 'Sistem' },
...
export function getActiveTabFromLocation(location: string): TabId {
  ...
  const match = (Object.entries(TAB_PATHS) as [TabId, string][]).find(([, routePath]) => routePath === path);
  if (match) return match[0];

  const legacyTabId = path.replace('/', '');
  return TABS.some((tab) => tab.id === legacyTabId) ? (legacyTabId as TabId) : 'dashboard';
}

export const PRIORITY_TABS: readonly TabId[] = ['dashboard', 'sales', 'products', 'kasa', 'cari'];
...
export function loadFavoriteTabs(): TabId[] {
  try {
    ...
    const favorites = parsed.filter(
      (tabId): tabId is TabId => typeof tabId === 'string' && validTabIds.has(tabId as TabId),
    );
