...
export async function parseExcelFile(file: File): Promise<ExcelFile> {
  ...
  const sheets: SheetData[] = workbook.sheetNames.map((sheetName) => {
    const rawData = workbook.getSheetRows(sheetName, { defval: null, blankrows: true }) as (
      | string
      | number
      | boolean
      | null
...
export async function parseCsvFile(file: File): Promise<ExcelFile> {
  ...
  const headers = rawData.length > 0 ? makeSafeHeaders(rawData[0] as (string | number | boolean | null)[]) : [];

  const rows = rawData.slice(1).map((rawRow) => headersToRow(headers, rawRow));

  return buildExcelResult(
    file,
    [{ name: 'Sayfa1', headers, rows, rawRows: rawData.slice(1) as (string | number | boolean | null)[][] }],
    'csv',
  );
}
