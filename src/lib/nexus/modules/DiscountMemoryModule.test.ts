...
function makeSale(partial: Partial<Sale> & { id: string; cariId: string; createdAt: string }): Sale {
  return {
    ...
    status: "tamamlandi",
    items: [
      { productId: "p1", productName: "Soba 80lik", quantity: 1, unitPrice: 5000, cost: 4000, total: 5000 },
    ],
    ...partial,
  } as Sale;
}
...
function makeTestDB(sales: Sale[], cariExtra: DB["cari"] = []): DB {
  return makeDB({
    products: [
      { id: "p1", name: "Soba 80lik", stock: 100, price: 5000, cost: 4000, category: "soba", minStock: 2, deleted: false, createdAt: "2026-01-01", updatedAt: "2026-01-01" },
    ],
    sales,
    cari: [
      { id: "c-ali", name: "Ali Yılmaz", balance: 0, deleted: false, type: "musteri", createdAt: "2026-01-01", updatedAt: "2026-01-01" },
      { id: "c-ahmet", name: "Ahmet Bey", balance: 0, deleted: false, type: "musteri", createdAt: "2026-01-01", updatedAt: "2026-01-01" },
      { id: "c-mehmet", name: "Mehmet Demir", balance: 0, deleted: false, type: "musteri", createdAt: "2026-01-01", updatedAt: "2026-01-01" },
      ...cariExtra,
    ],
  });
}
...
describe("DiscountMemoryModule / findCariByName", () => {
  ...
  it("silinmiş cari eşleşmez", () => {
    const db = makeTestDB([], [
      { id: "c-sil", name: "Silinmiş Kişi", balance: 0, deleted: true, type: "musteri", createdAt: "2026-01-01", updatedAt: "2026-01-01" },
    ]);
...
describe("DiscountMemoryModule / recallDiscountHistory", () => {
  ...
  it("iptal/iade edilen satışlar dahil edilmez", () => {
    const db = makeTestDB([
      makeSale({ id: "s1", cariId: "c-ali", createdAt: "2026-06-19", discount: 10, discountAmount: 500, status: "iptal" }),
    ]);
    const recall = recallDiscountHistory(db, "c-ali");
...
describe("DiscountMemoryModule / applyDiscountProposalToPayload", () => {
  it("geçerli öneriyi payload'a yazar (cariId + discount)", () => {
    const db = makeTestDB([
      makeSale({ id: "s-ali-1", cariId: "c-ali", createdAt: "2026-06-18", discount: 10, discountAmount: 500, total: 4500 }),
    ]);
    const proposal = proposeDiscountTransfer(db, "Ali", "Ahmet");
