...
import {
  ...
  buildUndoIntent,
  resolveUndo,
} from "./VoiceUndoEngine";

// ── Test yardımcıları ────────────────────────────────────────────────────────

function makeLogEntry(partial: Partial<AIActionLogEntry> & { id: string; createdAt: string; actionType: string }): AIActionLogEntry {
  return {
    model: "offline",
    mode: "manual",
    label: "Test işlem",
...
describe("VoiceUndoEngine / findLastUndoableAction", () => {
  ...
  it("filter sale ile sadece satışları", () => {
    const db = makeTestDB({
      aiActionLog: [
        makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "kasa_gider", affectedIds: ["k1"] }),
  ...
  it("blocked/failed kayıtları dahil etmez", () => {
    const db = makeTestDB({
      aiActionLog: [
        makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "sale", status: "blocked", affectedIds: ["s1"] }),
        makeLogEntry({ id: "l2", createdAt: "2026-06-19T10:00:00.000Z", actionType: "sale", status: "applied", affectedIds: ["s2"] }),
      ],
    });
    const entry = findLastUndoableAction(db);
    expect(entry?.id).toBe("l2");
  ...
  it("any filter tüm undoable tipler", () => {
    const db = makeTestDB({
      aiActionLog: [
        makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "kasa_gelir", affectedIds: ["k1"] }),
      ],
    });
...
describe("VoiceUndoEngine / buildUndoIntent", () => {
  it("sale → sale_iptal Intent", () => {
    ...
    const logEntry = makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "sale", affectedIds: ["s1"] });
  ...
  it("sale — affectedIds yoksa hata", () => {
    const db = makeTestDB();
    const logEntry = makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "sale", affectedIds: [] });
    const result = buildUndoIntent(db, logEntry);
  ...
  it("sale — DB'de yoksa hata", () => {
    const db = makeTestDB();
    const logEntry = makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "sale", affectedIds: ["yok"] });
    const result = buildUndoIntent(db, logEntry);
    expect(result.ok).toBe(false);
  ...
  it("kasa_gelir → kasa_gider (ters) Intent", () => {
    ...
    const logEntry = makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "kasa_gelir", affectedIds: ["k1"] });
    const result = buildUndoIntent(db, logEntry);
    expect(result.ok).toBe(true);
  ...
  it("kasa_gider → kasa_gelir (ters) Intent", () => {
    ...
    const logEntry = makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "kasa_gider", affectedIds: ["k1"] });
    const result = buildUndoIntent(db, logEntry);
    expect(result.ok).toBe(true);
  ...
  it("cari_tahsilat → henüz desteklenmiyor", () => {
    const db = makeTestDB();
    const logEntry = makeLogEntry({ id: "l1", createdAt: "2026-06-20T10:00:00.000Z", actionType: "cari_tahsilat", affectedIds: ["c1"] });
    const result = buildUndoIntent(db, logEntry);
    expect(result.ok).toBe(false);
