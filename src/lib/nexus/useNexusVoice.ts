...
export function useNexusVoice() {
  ...
  const listen = useCallback(async (onResult: (text: string) => void, onError?: (error: string) => void) => {
  ...
  const speakAndListen = useCallback(
    async (text: string, handleInput: (text: string) => Promise<void>, onError?: (error: string) => void) => {
