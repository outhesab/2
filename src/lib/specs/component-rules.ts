        ...
        try {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (
              m &&
              !lines[i].includes('--') &&
              !lines[i].includes('width') &&
              !lines[i].includes('pct') &&
              !lines[i].includes('percent') &&
              !lines[i].includes('background') &&
              !lines[i].includes('transform') &&
              !lines[i].includes('animation') &&
              !lines[i].includes('opacity')
            ) {
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (!content.includes("from '@/components/ui/empty'") && !content.includes('<Empty')) {
            violations.push({
              file,
              message: 'Empty component import edilmemiş — boş liste durumu yönetilmiyor olabilir',
            });
          }
        } catch {
