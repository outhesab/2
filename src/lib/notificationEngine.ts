...
export function generateNotifications(db: DB): AppNotification[] {
  ...
  if (kritikStok.length > 0) {
    kritikStok.slice(0, 4).forEach((p) => {
      add(
        ...
        '⚠️',
        `Kritik stok: ${p.name}`,
        `${p.stock} adet kaldı (min: ${p.minStock})`,
        'products',
        p.id,
      );
    });
    if (kritikStok.length > 4) {
      add(
        ...
        'stok',
        '⚠️',
        `${kritikStok.length - 4} ürün daha kritik seviyede`,
        'Stok sayfasını kontrol edin',
        'products',
      );
    }
  }
  ...
  if (buyukAlacaklar.length > 5) {
    ...
    add(
      ...
      'cari',
      '💼',
      `Toplam ${buyukAlacaklar.length} müşteri bakiye`,
      `Toplam alacak: ${formatMoney(toplam)}`,
      'cari',
    );
  }
  ...
  if (bekliyenSiparisler.length > 0) {
    add(
      ...
      'siparis',
      '🚚',
      `${bekliyenSiparisler.length} sipariş teslim bekliyor`,
      `Toplam: ${formatMoney(bekliyenSiparisler.reduce((s, o) => s + o.amount, 0))}`,
      'suppliers',
    );
  }
  ...
  if (odenmemisTamam.length > 0) {
    add(
      ...
      'siparis',
      '💳',
      `${odenmemisTamam.length} tamamlanan sipariş ödenmemiş`,
      `Toplam borç: ${formatMoney(odenmemisTamam.reduce((s, o) => s + o.remainingAmount, 0))}`,
      'suppliers',
    );
  }
  ...
  if (yaklasanVade.length > 0) {
    yaklasanVade.forEach((inv) => {
      ...
      add(
        ...
        'fatura',
        '🧾',
        `Fatura vadesi: ${inv.cariName}`,
        `#${inv.invoiceNo || inv.id.slice(-6)} · ${formatMoney(inv.total)} · Vade: ${label}`,
        'fatura',
        inv.id,
      );
    });
  ...
  if (gecmisVade.length > 0) {
    ...
    add(
      ...
      'fatura',
      '🔴',
      `${gecmisVade.length} fatura vadesi geçmiş`,
