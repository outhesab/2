...
export function computeAlacak(db: DB): number {
  return db.cari.filter((c) => !c.deleted && c.type === 'musteri' && c.balance > 0).reduce((s, c) => s + c.balance, 0);
}
...
export function computeKasaToplam(db: DB): number {
  return db.kasa.filter((k) => !k.deleted).reduce((s, k) => s + (k.type === 'gelir' ? k.amount : -k.amount), 0);
}
...
export function computeStokDeger(db: DB): number {
  return db.products.filter((p) => !p.deleted).reduce((s, p) => s + p.cost * p.stock, 0);
}
...
export function getLowStockProducts(db: DB) {
  return db.products.filter((p) => !p.deleted && p.stock > 0 && p.stock <= p.minStock);
}

export function getMonthSales(db: DB) {
  const monthStart = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
  const sales = db.sales.filter((s) => !s.deleted && s.status === 'tamamlandi' && new Date(s.createdAt) >= monthStart);
