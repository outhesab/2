...
function parseSayfa1(rows: unknown[][]): ImportPreview['sales'] {
  ...
  for (const r of rows) {
    if (!headerFound) {
      if (String(r[0]).toUpperCase().includes('TARİH') || String(r[0]).toUpperCase().includes('TARIH')) {
        headerFound = true;
      }
      continue;
    }
    ...
    const product =
      sobaQty > 0 && peletKg > 0 ? `Soba+Pelet` : sobaQty > 0 ? 'Soba' : peletKg > 0 ? 'Pelet' : desc || 'Soba';
    result.push({
      date: toISO(r[0]),
      customer,
      product,
...
export default function ExcelImport({ db, save }: Props) {
  ...
  const doImport = () => {
    if (!preview) return;
    const totalCount =
      (opts.cari ? preview.cari.length : 0) +
      (opts.sales ? preview.sales.length : 0) +
      (opts.giderler ? preview.giderler.length : 0);
    showConfirm(
      "Excel'den İçe Aktar",
      `Toplam ${totalCount} kayıt sisteme aktarılacak. Mevcut verilerle çakışma kontrolü yapılır ama bazı mükerrer kayıtlar oluşabilir. Devam edilsin mi?`,
      () => {
        ...
        save((prev) => {
          ...
          if (opts.giderler && preview.giderler.length > 0) {
            const kasaCatMap: Record<string, string> = {
              NAKLİYE: 'nakliye',
              PERSONEL: 'personel',
              'İŞLETME GİDERLERİ': 'isletme',
              'MARKET / ALIŞ VERİŞ': 'market',
              'PELET YÜKLEME': 'pelet_yukleme',
              'PELET BOŞALTMA': 'pelet_bosaltma',
              KASA: 'kasa_gider',
            };
        ...
        showToast(
          `✅ İçe aktarma tamamlandı! ${opts.cari ? preview.cari.length + ' cari' : ''} ${opts.sales ? preview.sales.length + ' satış' : ''} ${opts.giderler ? preview.giderler.length + ' gider' : ''} eklendi.`,
        );
        setPreview(null);
      },
    );
        ...
        <Card title="📥 Excel Dosyası Seç — SOBA TAKİP (.xlsm / .xlsx)">
          ...
          >
            ...
            {loading ? (
              ...
              <>
                <div style={{ color: '#93c5fd', fontWeight: 700, fontSize: '1rem', marginBottom: 6 }}>
                  Dosyayı buraya sürükleyin ya da tıklayın
                </div>
                <div style={{ color: '#475569', fontSize: '0.82rem' }}>
                  SOBA_TAKIP_GIDERLI.xlsm ve .xlsx formatları desteklenir
                </div>
                <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginTop: 10 }}>
                  {[
                    ['ÖZET TABLO', '→ Cariler', '#8b5cf6'],
                    ['Sayfa1', '→ Satışlar', '#10b981'],
                    ['GİDERLER', '→ Kasa', '#f59e0b'],
                  ].map(([s, t, c]) => (
                    <span
                      key={s}
                      style={{
      ...
      {preview && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
            {[
              { key: 'cari' as const, icon: '👥', label: 'Cari Müşteri', count: preview.cari.length, color: '#8b5cf6' },
              {
                ...
                icon: '🛒',
                label: 'Satış Kaydı',
                count: preview.sales.length,
                color: '#10b981',
              },
              {
                key: 'giderler' as const,
                icon: '💸',
                label: 'Gider Kaydı',
                count: preview.giderler.length,
                color: '#f59e0b',
              },
            ].map((s) => (
              <div
                ...
                style={{
                  background: opts[s.key] ? `${s.color}12` : 'rgba(0,0,0,0.2)',
                  borderRadius: 12,
                  padding: '14px',
                  border: `2px solid ${opts[s.key] ? s.color + '40' : 'transparent'}`,
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  textAlign: 'center',
                }}
              >
                ...
                <div style={{ fontSize: '1.4rem', fontWeight: 900, color: opts[s.key] ? s.color : '#334155' }}>
                  {s.count}
                </div>
          ...
          <Card title="👁️ Önizleme — aktarmak istediklerinizi seçmek için yukarıya tıklayın">
            ...
            >
              {[
                { id: 'cari' as const, label: `👥 Cariler (${preview.cari.length})` },
                { id: 'satis' as const, label: `🛒 Satışlar (${preview.sales.length})` },
                { id: 'gider' as const, label: `💸 Giderler (${preview.giderler.length})` },
              ].map((t) => (
                <button
                  key={t.id}
                  ...
                  style={{
                    ...
                    fontWeight: 700,
                    fontSize: '0.78rem',
                    background: activeTab === t.id ? 'linear-gradient(135deg,#ff5722,#ff7043)' : 'transparent',
            ...
            <div style={{ maxHeight: 280, overflowY: 'auto' }}>
              {activeTab === 'cari' && (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  <thead>
                    <tr>
                      {['Müşteri', 'Toplam Alış', 'Bakiye', 'Durum'].map((h) => (
                  ...
                  <tbody>
                    {preview.cari.map((c, i) => {
                      const existingCari = db.cari.find(
                        (x) => x.name.trim().toLowerCase() === c.name.trim().toLowerCase(),
                      );
                      return (
                        <tr
                          key={i}
                          style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', opacity: existingCari ? 0.5 : 1 }}
                        >
                          <td style={tdStyle}>
                            {existingCari && (
                              <span
                          ...
                          <td style={{ ...tdStyle }}>
                            <span style={{ color: durum2color(c.durum), fontSize: '0.78rem' }}>{c.durum}</span>
                          </td>
                        </tr>
                      );
                    })}
              ...
              {activeTab === 'satis' && (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  <thead>
                    <tr>
                      {['Tarih', 'Müşteri', 'Ürün', 'Adet', 'Tutar'].map((h) => (
                        <th key={h} style={thStyle}>
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {preview.sales.map((s, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        ...
                        <td style={{ ...tdStyle, color: '#10b981', fontWeight: 700, textAlign: 'right' }}>
                          {formatMoney(s.total)}
                        </td>
                      </tr>
                    ))}
                  ...
                  <tbody>
                    {preview.giderler.map((g, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        ...
                        <td style={{ ...tdStyle, color: '#ef4444', fontWeight: 700, textAlign: 'right' }}>
          ...
          {(() => {
            const existing = preview.cari.filter((c) =>
              db.cari.some((x) => x.name.trim().toLowerCase() === c.name.trim().toLowerCase()),
            );
            return existing.length > 0 ? (
              ...
              >
                ⚠️ <strong>{existing.length} müşteri</strong> zaten kayıtlı (tabloda "VAR" işaretli). Bunlar tekrar
                aktarılmayacak; sadece yeni {preview.cari.length - existing.length} müşteri eklenecek.
              </div>
            ) : null;
          })()}
          ...
          >
            ...
            >
              ...
              <span style={{ color: '#10b981', fontWeight: 800 }}>
                {formatMoney(preview.sales.reduce((s, x) => s + x.total, 0))}
              </span>
              <span style={{ color: '#64748b' }}>Toplam Gider:</span>
              <span style={{ color: '#ef4444', fontWeight: 800 }}>
                {formatMoney(preview.giderler.reduce((s, x) => s + x.amount, 0))}
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              onClick={doImport}
              disabled={!opts.cari && !opts.sales && !opts.giderler}
              style={{
                flex: 1,
                padding: '14px 0',
                background:
                  !opts.cari && !opts.sales && !opts.giderler
                    ? 'rgba(0,0,0,0.2)'
                ...
                color: '#fff',
                fontWeight: 900,
                cursor: !opts.cari && !opts.sales && !opts.giderler ? 'not-allowed' : 'pointer',
                fontSize: '0.95rem',
              }}
            >
              ✅{' '}
              {(opts.cari
                ? preview.cari.filter((c) => !db.cari.some((x) => x.name.toLowerCase() === c.name.toLowerCase())).length
                : 0) +
                (opts.sales ? preview.sales.length : 0) +
                (opts.giderler ? preview.giderler.length : 0)}{' '}
              Kaydı Sisteme Aktar
