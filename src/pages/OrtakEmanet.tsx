...
export default function OrtakEmanet({ db, save }: Props) {
  ...
  const activeEntries = useMemo(
    () => (db.ortakEmanetler || []).filter((entry) => !(entry as { deleted?: boolean }).deleted),
    [db.ortakEmanetler],
  );
  ...
  const partnerRows = useMemo(() => {
    return activePartners.map((partner) => {
      ...
      const emanet = entries.filter((entry) => entry.type === 'emanet').reduce((sum, entry) => sum + entry.amount, 0);
      const iade = entries.filter((entry) => entry.type === 'iade').reduce((sum, entry) => sum + entry.amount, 0);
  ...
  const saveEntry = () => {
    ...
    const description =
      form.note ||
      (form.type === 'emanet' ? `Ortak emanet: ${partner?.name || ''}` : `Ortak iade: ${partner?.name || ''}`);
    ...
    save((prev) => {
      ...
      const cari = prev.cari.map((item) =>
        item.ortak && item.partnerId === form.partnerId
          ? {
              ...item,
              balance: (item.balance || 0) + (form.type === 'emanet' ? amount : -amount),
  ...
  return (
    <div>
      <section style={cardStyle}>
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 10, marginTop: 16 }}
        >
      ...
      <section style={{ ...cardStyle, marginTop: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10 }}>
          ...
          <Field label="İşlem">
            <select
              value={form.type}
              onChange={(event) => setForm((prev) => ({ ...prev, type: event.target.value as 'emanet' | 'iade' }))}
              style={input}
            >
          ...
          <Field label="Kasa">
            <select
              value={form.kasa}
              onChange={(event) => setForm((prev) => ({ ...prev, kasa: event.target.value }))}
              style={input}
            >
              {(db.kasalar || []).map((kasa) => (
