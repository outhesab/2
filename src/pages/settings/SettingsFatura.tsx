    ...
    <div className="space-y-4">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                value={cfg.seriOnEki}
                onChange={(e) => update({ seriOnEki: e.target.value.slice(0, 3).toUpperCase() })}
            ...
            <div>
              ...
              <input
                type="number"
                min={1}
                value={cfg.baslangicNo}
                onChange={(e) => update({ baslangicNo: Math.max(1, parseInt(e.target.value) || 1) })}
                className="w-full mt-1 px-3 py-2 bg-card border border-white/10 rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-card border border-white/5">
            ...
            <button
              onClick={() => update({ otomatikNo: !cfg.otomatikNo })}
              className={`relative w-12 h-6 rounded-full transition-colors ${cfg.otomatikNo ? 'bg-indigo-600' : 'bg-gray-600'}`}
            >
              <span
                className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${cfg.otomatikNo ? 'left-6' : 'left-0.5'}`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-card border border-white/5">
            ...
            <button
              onClick={() => update({ kdvDahil: !cfg.kdvDahil })}
              className={`relative w-12 h-6 rounded-full transition-colors ${cfg.kdvDahil ? 'bg-indigo-600' : 'bg-gray-600'}`}
            >
              <span
                className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${cfg.kdvDahil ? 'left-6' : 'left-0.5'}`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-card border border-white/5">
            ...
            <button
              onClick={() => update({ eFatura: !cfg.eFatura })}
              className={`relative w-12 h-6 rounded-full transition-colors ${cfg.eFatura ? 'bg-indigo-600' : 'bg-gray-600'}`}
            >
              <span
                className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${cfg.eFatura ? 'left-6' : 'left-0.5'}`}
              />
            </button>
          </div>
          ...
          <div>
            ...
            <textarea
              value={cfg.varsayilanNot}
              onChange={(e) => update({ varsayilanNot: e.target.value })}
              rows={2}
              maxLength={500}
      ...
      <Card title="📄 Fatura Önizleme">
        <div className="p-4 rounded-lg bg-card border border-white/5 font-mono text-xs text-muted-foreground">
          ...
          <div>
            Seri: {cfg.seriOnEki || 'A'}-{String(cfg.baslangicNo).padStart(6, '0')}
          </div>
