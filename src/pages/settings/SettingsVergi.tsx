...
function loadVergi(): VergiAyarlari {
  ...
  } catch { logger.warn('settings', 'Vergi ayarlari yuklenemedi'); }
      ...
      <Card title="💰 KDV & Vergi Ayarları">
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-2 mt-1">
              {[0, 1, 8, 10, 18, 20].map((o) => (
                <button key={o} onClick={() => update({ kdvOrani: o })}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    cfg.kdvOrani === o ? 'bg-indigo-600 text-white shadow-lg' : 'bg-card border border-white/10 text-muted-foreground hover:text-foreground'
                  }`}>{o}%</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1">
              {([['TRY', '₺'], ['USD', '$'], ['EUR', '€']] as const).map(([k, s]) => (
                <button key={k} onClick={() => update({ paraBirimi: k as 'TRY' | 'USD' | 'EUR' })}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    cfg.paraBirimi === k ? 'bg-indigo-600 text-white shadow-lg' : 'bg-card border border-white/10 text-muted-foreground hover:text-foreground'
                  }`}>{s} {k}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1">
              {([['nakit', '💵 Nakit'], ['kart', '💳 Kart'], ['havale', '🏦 Havale/EFT']] as const).map(([k, l]) => (
                <button key={k} onClick={() => update({ varsayilanOdeme: k as 'nakit' | 'kart' | 'havale' })}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    cfg.varsayilanOdeme === k ? 'bg-indigo-600 text-white shadow-lg' : 'bg-card border border-white/10 text-muted-foreground hover:text-foreground'
                  }`}>{l}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-card border border-white/5">
            ...
            <button onClick={() => update({ yuvarlama: !cfg.yuvarlama })}
              className={`relative w-12 h-6 rounded-full transition-colors ${cfg.yuvarlama ? 'bg-indigo-600' : 'bg-gray-600'}`}>
              <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${cfg.yuvarlama ? 'left-6' : 'left-0.5'}`} />
            </button>
          </div>
        </div>
      </Card>
