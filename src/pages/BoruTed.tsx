...
export default function BoruTed({ db, save }: Props) {
  ...
  return (
    <div>
      ...
      {tab === 'orders' && (
        <div className="responsive-table-wrap" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01))', borderRadius: 14, border: '1px solid rgba(255,255,255,0.07)', overflowX: 'auto' }}>
            ...
            <tbody>
              {db.boruOrders.length === 0 ? (
                ...
                <tr key={o.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                  <td data-label="Tarih" style={{ padding: '12px 16px', color: '#475569', fontSize: '0.82rem' }}>{formatDate(o.createdAt)}</td>
                  <td data-label="Tedarikçi" style={{ padding: '12px 16px', color: '#f1f5f9', fontWeight: 600 }}>{db.boruSuppliers.find(s => s.id === o.supplierId)?.name || '-'}</td>
