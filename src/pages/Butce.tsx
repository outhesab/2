...
const PRESET_CATEGORIES: Omit<BudgetCategory, 'id'>[] = [
  ...
  {
    name: 'Elektrik/Su/Gaz',
    icon: '⚡',
    monthlyLimit: 1500,
    color: '#f59e0b',
    kasaCategories: ['elektrik', 'su', 'gaz', 'fatura'],
  },
  {
    name: 'Personel Maaş',
    icon: '👥',
    monthlyLimit: 20000,
    color: '#8b5cf6',
    kasaCategories: ['maas', 'personel', 'maaş'],
  },
  {
    name: 'Ham Madde/Stok',
    icon: '📦',
    monthlyLimit: 15000,
    color: '#10b981',
    kasaCategories: ['alis', 'stok', 'hammadde', 'alis_fatura'],
  },
  {
    name: 'Nakliye/Kargo',
    icon: '🚚',
    monthlyLimit: 3000,
    color: '#06b6d4',
    kasaCategories: ['nakliye', 'kargo', 'tasima'],
  },
  {
    ...
    monthlyLimit: 2000,
    color: '#ec4899',
    kasaCategories: ['reklam', 'pazarlama', 'tanitim'],
  },
  {
    name: 'Diğer Giderler',
    icon: '📋',
...
export default function Butce({ db, save }: Props) {
  ...
  const [importResult, setImportResult] = useState<{
    total: number;
    matched: number;
    entries: Array<{ date: string; desc: string; amount: number; type: 'gelir' | 'gider' }>;
  } | null>(null);
  const fileRef = useRef<HTMLInputElement>(null);
  const [form, setForm] = useState<Omit<BudgetCategory, 'id'>>({
    name: '',
    icon: '📋',
    monthlyLimit: 0,
    color: 'var(--text-muted)',
  ...
  const monthKasa = useMemo(
    () => db.kasa.filter((k) => !k.deleted && k.createdAt >= startOfMonth && k.createdAt <= endOfMonth),
    [db.kasa, startOfMonth, endOfMonth],
  );
  ...
  const categorySpend = useMemo(() => {
    ...
    return budgets.map((b) => {
      ...
      return {
        ...
        pct: b.monthlyLimit > 0 ? Math.min(100, (spent / b.monthlyLimit) * 100) : 0,
      };
    });
  ...
  const initPresets = () => {
    showConfirm(
      'Hazır Kategoriler',
      'Standart gider kategorileri eklenecek. Mevcut kategoriler korunacak. Devam edilsin mi?',
      () => {
        save((prev) => {
  ...
  const openAdd = () => {
    setForm({ name: '', icon: '📋', monthlyLimit: 0, color: 'var(--text-muted)', kasaCategories: [] });
    setEditId(null);
    setModal(true);
  };
  const openEdit = (b: BudgetCategory) => {
    setForm({
      ...
      color: b.color,
      kasaCategories: [...(b.kasaCategories || [])],
    });
    setEditId(b.id);
    setModal(true);
  };
  ...
  const parseBankCSV = (text: string) => {
    ...
    lines.forEach((line) => {
      ...
      const desc = cols
        .filter((c) => c !== dateStr && c !== amtStr && c.length > 2)
        .join(' — ')
        .slice(0, 120);
  ...
  const handleFileImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const processText = (text: string) => {
      ...
      if (entries.length === 0) {
        showToast('Uyumlu veri bulunamadı. Dosya formatını kontrol edin!', 'error');
        return;
      }
      let matched = 0;
      entries.forEach((e) => {
        const matchesKw = (kc: string) => {
          const kl = kc.toLowerCase();
          return e.desc
            .toLowerCase()
            .split(/\s+/)
  ...
  const confirmImport = () => {
    ...
    save((prev) => {
      const newEntries = importResult.entries.map((e) => {
        const cat = budgets.find((b) =>
          (b.kasaCategories || []).some((kc) => {
            const kl = kc.toLowerCase();
            const dl = e.desc.toLowerCase();
        ...
        return {
          id: genId(),
          type: e.type,
          category: (cat?.kasaCategories || [])[0] || 'banka_ekstere',
          amount: e.amount,
          kasa: 'banka' as const,
    ...
    if (failedDateCount > 0) {
      showToast(
        `✅ ${importResult.total} işlem aktarıldı. ⚠️ ${failedDateCount} kaydın tarihi okunamadı, bugünün tarihi kullanıldı.`,
        'success',
      );
    } else {
    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      >
        {[
          ...
          { icon: '💡', label: 'Toplam Bütçe', value: formatMoney(totalBudget), color: '#3b82f6' },
          {
            icon: '⚖️',
            label: 'Bütçe Kalan',
            value: formatMoney(totalBudget - totalExpense),
            color: totalBudget - totalExpense >= 0 ? '#10b981' : '#ef4444',
          },
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {budgets.length === 0 && (
            <button onClick={initPresets} style={btnSecondary}>
              ✨ Hazır Kategoriler
            </button>
          )}
          <button
          ...
          <button
            onClick={() => {
              showConfirm(
                'Bütçeyi Kopyala',
                `Mevcut kategoriler (${budgets.length} adet) yeni yıla kopyalansın mı?`,
                () => {
      ...
      {yearlyData.some((d) => d.gelir > 0 || d.gider > 0) && (
        ...
        >
          <div style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '0.9rem', marginBottom: 14 }}>
            📅 {year} Yıllık Gelir / Gider
          </div>
      ...
      {budgets.length === 0 ? (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', marginTop: 12 }}>
            <button onClick={initPresets} style={btnPrimary}>
              ✨ Hazır Kategoriler Yükle
            </button>
          ...
          {(categorySpend || []).map((b) => (
            ...
            >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                      <span
                        style={{
                          color: b.pct >= 90 ? '#ef4444' : b.pct >= 70 ? '#f59e0b' : '#10b981',
                          fontWeight: 800,
                          fontSize: '0.88rem',
                        }}
                      >
                      ...
                      <button onClick={() => openEdit(b)} style={miniBtn}>
                        ✏️
                      </button>
                      <button onClick={() => deleteCat(b.id)} style={{ ...miniBtn, color: '#ef4444' }}>
                        🗑️
                      </button>
                    </div>
                  </div>
                        ...
                        background:
                          b.pct >= 90
                            ...
                            : b.pct >= 70
                              ...
                              : `linear-gradient(90deg, ${b.color}, ${b.color}aa)`,
                        borderRadius: 6,
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                ...
                >
                  Kalan:{' '}
                  <strong style={{ color: b.remaining >= 0 ? '#10b981' : '#ef4444' }}>
                    {formatMoney(Math.abs(b.remaining))}
                    {b.remaining < 0 ? ' aşıldı!' : ''}
                  </strong>
                </span>
                <span
                  style={{
                ...
                >
                  %{b.pct.toFixed(0)} kullanıldı
                </span>
                {(b.kasaCategories || []).map((kc) => (
                  <span
                    key={kc}
                    style={{
          ...
          {uncategorized.length > 0 && (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div style={{ color: '#f59e0b', fontWeight: 700, fontSize: '0.88rem' }}>
                  ⚠️ Kategorisiz Giderler ({uncategorized.length} işlem)
                </div>
                <span style={{ color: '#ef4444', fontWeight: 800 }}>
                  {formatMoney(uncategorized.reduce((s, k) => s + k.amount, 0))}
                </span>
              </div>
              ...
              {uncategorized.length > 5 && (
                <p style={{ color: '#475569', fontSize: '0.78rem', marginTop: 6 }}>
                  ...ve {uncategorized.length - 5} işlem daha
                </p>
              )}
            </div>
          )}
      ...
      <Modal
        ...
        title={editId ? '✏️ Kategori Düzenle' : '+ Bütçe Kategorisi Ekle'}
        maxWidth={480}
      >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: 10 }}>
            <div>
              ...
              <input
                ...
                style={{ ...inp, textAlign: 'center', fontSize: '1.5rem' }}
              />
            </div>
            <div>
              <label style={lbl}>Kategori Adı *</label>
              <input
                value={form.name}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px', gap: 10 }}>
            ...
            <div>
              ...
              <input
                ...
                style={{ ...inp, padding: 6, cursor: 'pointer' }}
              />
            </div>
          </div>
          <div>
            ...
            <input
              value={form.kasaCategories.join(', ')}
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  kasaCategories: e.target.value
                    .split(',')
                    .map((s) => s.trim())
                    .filter(Boolean),
                }))
              ...
              style={inp}
              placeholder="elektrik, enerji, fatura"
            />
            <p style={{ color: '#334155', fontSize: '0.73rem', marginTop: 5 }}>
              Kasa işlemlerinin açıklama/kategori alanında bu kelimeler aranacak
            </p>
          </div>
          <button
            onClick={handleSave}
      ...
      >
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          >
            ...
            Her satırda: <strong>Tarih, Açıklama, Tutar</strong> (virgül / noktalı virgül / tab ayrımlı veya Excel
            sütunları)
            <br />
            <em style={{ color: 'var(--text-muted)' }}>
              İpucu: Excel'den "Farklı Kaydet → CSV" veya doğrudan .xlsx yükleyebilirsiniz.
            </em>
          </div>
          {!importResult ? (
            <>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                {[
                  { label: 'Toplam İşlem', value: importResult.total, color: '#3b82f6' },
                  { label: 'Otomatik Eşleşen', value: importResult.matched, color: '#10b981' },
                  { label: 'Eşleşmeyen', value: importResult.total - importResult.matched, color: '#f59e0b' },
                ].map((s) => (
                  <div
                    key={s.label}
                    style={{ background: `${s.color}12`, borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}
                  >
              ...
              >
                {importResult.entries.slice(0, 15).map((e, i) => (
                  ...
                  >
                    ...
                    <span style={{ color: e.type === 'gelir' ? '#10b981' : '#ef4444', fontWeight: 700, marginLeft: 8 }}>
                      {e.type === 'gelir' ? '+' : '-'}
                      {formatMoney(e.amount)}
                    </span>
                  </div>
                ))}
                {importResult.entries.length > 15 && (
                  <p style={{ color: '#334155', fontSize: '0.75rem', textAlign: 'center', padding: '6px 0' }}>
                    ...ve {importResult.entries.length - 15} işlem daha
                  </p>
                )}
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                >
                  ✅ {importResult.total} İşlemi Kasa'ya Aktar
                </button>
                <button
