...
function StatusPill({ status }: { status: string }) {
  const col =
    status === 'tamamlandi' || status === 'ok'
      ? '#22c55e'
      : status === 'yolda' || status === 'warn'
        ? '#ff6b35'
        : status === 'bekliyor' || status === 'critical'
          ? 'var(--color-danger)'
          : '#787890';
  return (
    <span
...
export default function DashboardOperasyon({ db, onTabChange: _onTabChange }: Props) {
  ...
  const stockAlerts = useMemo(() => {
    return db.products
      .filter((p) => !p.deleted && p.stock <= p.minStock)
  ...
  const avgDailySales = useMemo(() => {
    ...
    db.sales
      .filter((s) => !s.deleted && s.status === 'tamamlandi' && s.createdAt >= cutoff && s.productName)
      .forEach((s) => {
  ...
  const orderPipeline = useMemo(() => {
    const groups: Record<string, { count: number; total: number }> = {
      bekliyor: { count: 0, total: 0 },
      yolda: { count: 0, total: 0 },
      tamamlandi: { count: 0, total: 0 },
      iptal: { count: 0, total: 0 },
    };
    db.orders
      .filter((o) => !o.deleted)
      .forEach((o) => {
        if (groups[o.status]) {
          ...
          groups[o.status].total += o.amount;
        }
      });
  ...
  const systemHealth = useMemo(() => {
    ...
    try {
      ...
      logger.warn('dashboard', 'localStorage okuma hatası, storagePct 0');
      storagePct = 0;
    }
    ...
    const totalRecords =
      db.products.length + db.sales.length + db.cari.length + db.kasa.length + db.orders.length + db.suppliers.length;
  ...
  const pendingActions = useMemo(() => {
    ...
    if (bekleyenOrders.length > 0)
      items.push({
        icon: '📋',
        text: 'Bekleyen sipariş',
        count: bekleyenOrders.length,
        amount: bekleyenOrders.reduce((s, o) => s + o.amount, 0),
        severity: 'warn',
      });
    const outStock = db.products.filter((p) => !p.deleted && p.stock === 0);
    if (outStock.length > 0)
      items.push({ icon: '⚠️', text: 'Stoğu biten ürün', count: outStock.length, amount: 0, severity: 'critical' });
    ...
    if (eskiAlacak.length > 0)
      items.push({
        icon: '🔴',
        text: '30+ gün gecikmiş alacak',
        count: eskiAlacak.length,
        amount: eskiAlacak.reduce((s, c) => s + c.balance, 0),
        severity: 'critical',
      });
    return items;
  }, [db]);
  ...
  const stockChart = useMemo(() => {
    ...
    stockAlerts.slice(0, 10).forEach((p) => {
      map[p.name] = { stock: p.stock, min: p.minStock };
    });
  ...
  return (
    ...
    >
      ...
      <motion.div variants={cardVariants} style={{ marginBottom: 12 }}>
        ...
        >
          <div>
            ...
            <p style={{ color: MUTED, fontSize: '0.72rem', fontFamily: MONO }}>
              {new Date().toLocaleString('tr-TR')} · {systemHealth.totalRecords} kayıt
            </p>
          </div>
            ...
            {filters.map((f) => (
              <motion.button
                ...
                style={{
                  padding: '3px 10px',
                  borderRadius: 4,
                  border: `1px solid ${filter === f ? '#ff6b35' : BORDER}`,
                  background: filter === f ? '#ff6b3518' : 'transparent',
                  color: filter === f ? '#ff6b35' : MUTED,
                  cursor: 'pointer',
                  fontSize: '0.7rem',
                  fontFamily: MONO,
      ...
      {(filter === 'all' || filter === 'stok') && (
        <OpCard
          title={`STOK KONTROL ODASI (${stockAlerts.length})`}
          subtitle="Stok seviyesi minStok altında olan ürünler"
          extra={<StatusPill status={stockAlerts.length > 0 ? 'critical' : 'ok'} />}
        >
          {stockAlerts.length === 0 ? (
            <Empty>
              <EmptyHeader>
                <EmptyMedia>📦</EmptyMedia>
                <EmptyTitle>Tüm stoklar yeterli</EmptyTitle>
              </EmptyHeader>
            </Empty>
          ) : (
            <>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem' }}>
                  <thead>
                    <tr style={{ color: MUTED, borderBottom: `1px solid ${BORDER}` }}>
                      ...
                      <th style={{ textAlign: 'right', padding: '6px 8px', fontWeight: 500, fontFamily: MONO }}>
                        Stok
                      </th>
                      ...
                      return (
                        <motion.tr
                          ...
                          style={{
                            ...
                            background: critical ? '#ef444408' : 'transparent',
                          }}
                        >
                          <td
                            style={{
                              padding: '7px 8px',
                              color: critical ? 'var(--color-danger)' : TEXT,
                              fontWeight: 500,
                            }}
                          >
                            {p.name}
                          ...
                          <td
                            style={{
                              ...
                              textAlign: 'right',
                              fontFamily: MONO,
                              color: critical ? 'var(--color-danger)' : p.stock <= p.minStock / 2 ? '#ff6b35' : TEXT,
                            }}
                          >
                            {p.stock}
                          </td>
                          <td style={{ padding: '7px 8px', textAlign: 'right', fontFamily: MONO, color: MUTED }}>
                            {p.minStock}
                          </td>
                          <td
                            style={{
                              ...
                              textAlign: 'right',
                              fontFamily: MONO,
                              color: depletionDays < 7 ? 'var(--color-danger)' : depletionDays < 30 ? '#ff6b35' : MUTED,
                            }}
                          >
                            {critical ? 'BİTTİ' : `${depletionDays}g`}
                          </td>
                          <td style={{ padding: '7px 8px', textAlign: 'right', fontFamily: MONO, color: MUTED }}>
                            {formatMoney(p.cost * p.stock)}
                          </td>
                        </motion.tr>
                      );
                    })}
                ...
                <div style={{ marginTop: 10 }}>
                  <ResponsiveContainer width="100%" height={100}>
                    <BarChart data={stockChart} margin={{ top: 4, right: 4, bottom: 4, left: -8 }}>
                      <XAxis
                        dataKey="name"
                        tick={{ fontSize: 9, fill: MUTED, fontFamily: MONO }}
                        axisLine={false}
                        tickLine={false}
                      />
                      ...
                      <Tooltip
                        contentStyle={{ background: CARD_BG, border: `1px solid ${BORDER}`, fontSize: '0.72rem' }}
                      />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
      ...
      {(filter === 'all' || filter === 'stok') &&
        (() => {
          ...
          const reorderUrgency = db.products
          ...
          return (
            <OpCard
              title="BUGÜN SİPARİŞ VER"
              subtitle="Acil yeniden sipariş listesi"
              extra={
                <span style={{ color: '#ff6b35', fontFamily: MONO, fontWeight: 700 }}>{reorderUrgency.length}</span>
              }
              style={{ marginTop: 12 }}
            >
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ color: MUTED, fontSize: '0.68rem', fontFamily: MONO }}>
                          Stok: {p.stock} · Günlük: {p.dailySales.toFixed(1)} ·{' '}
                          {p.daysLeft < 30 ? `${Math.floor(p.daysLeft)} gün kaldı` : 'Yeterli'}
                        </div>
                      </div>
                      <div style={{ width: 40, height: 4, borderRadius: 2, background: BORDER, overflow: 'hidden' }}>
                        <div
                          style={{
                            width: `${Math.min(100, p.urgencyScore)}%`,
                            height: 4,
                            borderRadius: 2,
                      ...
                      <span style={{ color: urgencyColor, fontFamily: MONO, fontSize: '0.72rem', fontWeight: 700 }}>
                        {Math.round(p.urgencyScore)}%
                      </span>
                    </motion.div>
                  );
                })}
          ...
          {orderPipeline.map((o) => {
            ...
            return (
              <OpCard
                ...
                extra={
                  <span style={{ fontFamily: MONO, color: col, fontSize: '1.2rem', fontWeight: 700 }}>{o.count}</span>
                }
              >
      ...
      {(filter === 'all' || filter === 'aktivite') && (
        <OpCard
          ...
          subtitle="Son 12 aktivite"
          extra={activityLog.length > 0 ? <StatusPill status={`${activityLog.length}`} /> : undefined}
          style={{ marginTop: 12 }}
        >
          {activityLog.length === 0 ? (
            <Empty>
              <EmptyHeader>
                <EmptyMedia>📋</EmptyMedia>
                <EmptyTitle>Aktivite yok</EmptyTitle>
              </EmptyHeader>
            </Empty>
          ) : (
                ...
                return (
                  <motion.div
                    ...
                    style={{
                      ...
                      gap: 10,
                      padding: '7px 4px',
                      borderBottom: i < activityLog.length - 1 ? `1px solid ${BORDER}` : 'none',
                      fontSize: '0.75rem',
                    }}
                  >
                    <span style={{ fontSize: '0.9rem' }}>
                      {a.action?.includes('satış') || a.action?.includes('satis')
                        ? '🛒'
                        : a.action?.includes('üret') || a.action?.includes('urun')
                          ? '📦'
                          : a.action?.includes('sil')
                            ? '🗑️'
                            : a.action?.includes('düz') || a.action?.includes('günc')
                              ? '✏️'
                              : '📝'}
                    </span>
                    <div style={{ flex: 1 }}>
                    ...
                    <span style={{ color: MUTED, fontFamily: MONO, fontSize: '0.65rem', whiteSpace: 'nowrap' }}>
                      {relStr}
                    </span>
                  </motion.div>
                );
        ...
        <OpCard
          ...
          extra={
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <StatusPill
                status={
                  systemHealth.daysSinceBackup === null || systemHealth.daysSinceBackup >= 7
                    ? 'critical'
                    : systemHealth.daysSinceBackup >= 3
                      ? 'warn'
                      : 'ok'
                }
              />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12 }}>
            <div>
              ...
              <div style={{ background: BORDER, borderRadius: 4, height: 8, overflow: 'hidden' }}>
                <div
                  style={{
                    width: `${systemHealth.storagePct}%`,
                    height: 8,
                    borderRadius: 4,
                    background:
                      systemHealth.storagePct > 70
                        ? 'var(--color-danger)'
                        : systemHealth.storagePct > 40
                          ? '#ff6b35'
                          : '#22c55e',
              ...
              <div style={{ color: TEXT, fontSize: '0.8rem', fontFamily: MONO, marginTop: 2 }}>
                %{systemHealth.storagePct.toFixed(1)}
              </div>
            </div>
            <div>
              <div
            ...
            <div>
              ...
              <div style={{ color: TEXT, fontSize: '0.8rem', fontFamily: MONO }}>
                {systemHealth.totalRecords.toLocaleString('tr-TR')}
              </div>
            </div>
          </div>
        </OpCard>
            ...
            {pendingActions.map((a, i) => (
              <motion.div
                ...
                style={{
                  ...
                  background:
                    a.severity === 'critical' ? '#ef444408' : a.severity === 'warn' ? '#ff6b3508' : '#22c55e08',
                  fontSize: '0.78rem',
                }}
              >
                <span>{a.icon}</span>
                <span style={{ color: TEXT }}>
                  {a.text}: <strong style={{ fontFamily: MONO }}>{a.count}</strong>
                </span>
