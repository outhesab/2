...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span style={{ background: c.bg, color: c.fg, fontSize: "0.7rem", padding: "2px 8px", borderRadius: 9999, fontWeight: 600 }}>
      {severity}
    </span>
  );
}
...
export default function SpecDashboard() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="visible" style={{ padding: 24, maxWidth: 960, margin: "0 auto" }}>
      <motion.div variants={item}>
        <h1 style={{ color: TEXT_PRIMARY, fontSize: "1.5rem", fontWeight: 700, marginBottom: 4 }}>
          Spec Compliance
        </h1>
...
function SpecCard({ name, ruleCount }: { name: string; count: number }) {
  ...
  return (
    <motion.div variants={item} style={{ background: BG_CARD, borderRadius: 12, padding: 20, marginBottom: 16, border: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      {rules.map((rule) => (
        <div key={rule.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
