...
function MagazineCard({
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.01, boxShadow: 'var(--shadow-lg, 0 8px 40px rgba(0,0,0,0.1))' }}
      style={{
...
function KpiBadge({ label, value, color = TEAL, sub }: { label: string; value: string; color?: string; sub?: string }) {
  return (
    <div style={{ minWidth: 120, padding: '10px 14px', borderRight: `1px solid ${BORDER}` }}>
      ...
      <div style={{ color, fontSize: '1.2rem', fontWeight: 700, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        {value}
      </div>
...
export default function DashboardTicaret({ db, onTabChange: _onTabChange }: Props) {
  ...
  const kpi = useMemo(() => {
    ...
    completed.forEach((s) => {
      prodRev[s.productName] = (prodRev[s.productName] || 0) + s.total;
    });
  ...
  const paymentData = useMemo(() => {
    ...
    db.sales
      .filter((s) => !s.deleted && s.status === 'tamamlandi' && s.createdAt >= cutoff)
      .forEach((s) => {
  ...
  return (
    ...
    >
      <motion.div variants={cardVariants} style={{ marginBottom: 16 }}>
        <div style={{ color: TEXT, fontSize: '1.3rem', fontWeight: 700, letterSpacing: '0.01em', marginBottom: 2 }}>
          Ticaret Zirvesi
        </div>
        <div style={{ color: MUTED, fontSize: '0.8rem' }}>
          Satış ve müşteri analitiği · {formatDate(new Date().toISOString())}
        </div>
      </motion.div>
      ...
      >
        ...
        <KpiBadge
          ...
          color={kpi.returnRate > 10 ? 'var(--color-danger)' : TEAL}
        />
      </motion.div>
          ...
          {topProducts.length === 0 ? (
            <Empty>
              <EmptyHeader>
                ...
                <EmptyTitle>Satış verisi yok</EmptyTitle>
              </EmptyHeader>
            </Empty>
          ) : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={topProducts} layout="vertical" margin={{ top: 4, right: 16, bottom: 4, left: 80 }}>
                ...
                />
                <Tooltip
                  formatter={(v: number, n: string) => [formatMoney(v), n === 'revenue' ? 'Ciro' : n]}
        ...
        <MagazineCard title="Kategori Dağılımı" subtitle="Ciro bazında">
          {categoryData.length === 0 ? (
            <Empty>
              <EmptyHeader>
                <EmptyMedia>🍩</EmptyMedia>
                <EmptyTitle>Kategori verisi yok</EmptyTitle>
              </EmptyHeader>
            </Empty>
          ) : (
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                    {days.map((d) => (
                      <div
                        key={d}
                        style={{ flex: 1, textAlign: 'center', fontSize: '0.65rem', color: MUTED, fontWeight: 600 }}
                      >
                        {d}
                      </div>
                    ))}
                    ...
                    <div key={h} style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                      {days.map((day) => {
                        ...
                        return (
                          <div
                            key={`${day}-${h}`}
                            title={`${day} ${h}: ${formatMoney(val)}`}
                            style={{
                              flex: 1,
                              height: 24,
                              borderRadius: 3,
                              background: `rgba(13,115,119,${intensity})`,
                              display: 'flex',
                              alignItems: 'center',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        <MagazineCard title="Ödeme Yöntem Trendi" subtitle="Son 14 gün">
          ...
          >
            <span>
              <span style={{ color: 'var(--color-success)' }}>●</span> Nakit
            </span>
            <span>
              <span style={{ color: 'var(--color-info)' }}>●</span> Kart
            </span>
            <span>
              <span style={{ color: 'var(--color-warning)' }}>●</span> Havale
            </span>
            <span>
              <span style={{ color: 'var(--color-accent)' }}>●</span> Cari
            </span>
          </div>
        </MagazineCard>
        ...
        <MagazineCard title="Toplam Müşteriler" subtitle="En yüksek bakiyeli 5 müşteri">
          {topCustomers.length === 0 ? (
            <Empty>
              <EmptyHeader>
                <EmptyMedia>👤</EmptyMedia>
                <EmptyTitle>Müşteri verisi yok</EmptyTitle>
              </EmptyHeader>
            </Empty>
          ) : (
                ...
                >
                  <div>
                    ...
                    <div style={{ color: MUTED, fontSize: '0.72rem' }}>
                      {c.lastTransaction ? formatDate(c.lastTransaction) : 'İşlem yok'}
                    </div>
                  </div>
