...
function StatCard({ icon, label, value, color, gradient, sub, onClick, trend }: {
  ...
  return (
    ...
    >
      ...
      <div className="dash-statcard-accent-line" style={{ background: `linear-gradient(90deg, transparent, ${color}60, transparent)` }} />
      ...
      {onClick && <motion.div className="dash-statcard-arrow" style={{ color: `${color}50` }} whileHover={{ x: 3, color }} transition={{ type: 'spring', stiffness: 300 }}>→</motion.div>}
    </motion.div>
  );
}
...
function ScrollableCards({ cards, onTabChange }: { cards: StatCardData[]; onTabChange: (tab: string) => void }) {
  ...
  return (
    <div className="dash-scroll-wrap">
      <button className={`dash-nav-btn dash-nav-btn-left ${canLeft ? 'visible' : 'hidden'}`} onClick={() => scroll(-1)}>‹</button>
      <button className={`dash-nav-btn dash-nav-btn-right ${canRight ? 'visible' : 'hidden'}`} onClick={() => scroll(1)}>›</button>
      <div
        ref={scrollRef}
...
function saveDashboardPrefs(prefs: { leftWidgets: WidgetId[]; brightness: number }) {
  ...
  }).catch(() => logger.error('sync', 'Dashboard prefs Firebase\'e yazılamadı'));
}
...
export default function Dashboard({ db, onTabChange, save }: Props) {
  ...
  const doRestore = async (backupId: string) => {
    ...
    await saveBackupToFirebase(db, preRestoreLabel).catch(() => logger.error('db', 'Geri yükleme öncesi yedek alınamadı'));
    ...
    if (restored) {
      ...
      setTimeout(() => saveBackupToFirebase(restored, `restore_sonrasi_${backupId.slice(0, 20)}`).catch(() => logger.error('db', 'Geri yükleme sonrası yedek alınamadı')), 1500);
      ...
      case 'recentSales':
        return (
          <WidgetCard title="🛒 Son Satışlar" subtitle={`${db.sales.length} toplam kayıt`} extra={
            <motion.button onClick={() => onTabChange('sales')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="dash-son-satislar-btn">Tümü →</motion.button>
          }>
            {recentSales.length === 0 ? (
              ...
              >
                ...
                <div className="dash-sale-amount">
                  <div className={`dash-sale-total ${s.status === 'tamamlandi' ? 'completed' : ''}`}>{formatMoney(s.total)}</div>
      ...
      case 'stockAlerts':
        ...
        return (
          <WidgetCard title="⚠️ Stok Uyarıları" subtitle="" extra={
            <motion.button onClick={() => onTabChange('products')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="dash-stok-uyari-btn">Görüntüle →</motion.button>
          }>
            {stats.outOfStock > 0 && (
            ...
            {stats.lowStock > 0 && (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="dash-stock-alert-row">
                <motion.div animate={{ scale: [1, 1.3, 1] }} transition={{ repeat: Infinity, duration: 2.5, ease: 'easeInOut', delay: 0.5 }} className="dash-stock-alert-dot" style={{ background: '#f59e0b' }} />
                <span className="dash-stock-alert-text"><strong className="dash-stock-alert-strong warning">{stats.lowStock} üründe</strong> az stok uyarısı</span>
              </motion.div>
            )}
          </WidgetCard>
        );
        ...
        return (
          <div className="dash-excel-bar">
            ...
            {[
              ...
              <motion.button
                key={btn.label}
                onClick={() => exportToExcel(db, { sheets: btn.sheets })}
                whileHover={{ scale: 1.04, background: `${btn.color}25` }}
                whileTap={{ scale: 0.96 }}
                className="dash-excel-btn" style={{ background: `${btn.color}15`, border: `1px solid ${btn.color}30`, color: btn.color }}
              >
                {btn.label}
              </motion.button>
            ))}
          ...
          <WidgetCard title="🍩 Kategori Dağılımı" subtitle="Satış kategorileri">
            <ResponsiveContainer width="100%" height={190}>
              <BarChart data={categoryRevenue.slice(0, 6)} margin={{ top: 8, right: 8, bottom: 4, left: -8 }}>
                ...
                <YAxis tick={chartAxisStyle} axisLine={false} tickLine={false} tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(0)}K` : String(v)} />
                <Tooltip
          ...
          try {
            ...
            return (
              <div className="dash-storage-box" style={{ border: `1px solid ${color}30` }}>
                ...
                <span className="dash-storage-value" style={{ color }}>{sizeMB.toFixed(2)} MB</span>
      ...
      {backupPanel && (
        <div className="dash-backup-panel">
          <div className="dash-backup-header">
            ...
            <button onClick={() => setBackupPanel(false)} className="dash-backup-close">✕</button>
          </div>
          ...
          {backupMsg && <div className={`dash-backup-msg ${backupMsg.startsWith('✅') ? 'success' : 'error'}`}>{backupMsg}</div>}
            ...
            {backups.map((b, i) => (
              ...
              >
                <div>
                  ...
                  <div className="dash-backup-item-meta">v{b.version} · {b.createdAt ? new Date(b.createdAt).toLocaleString('tr-TR') : ''}</div>
                </div>
                <motion.button
            ...
            <div className="dash-quick-grid">
              {[
                ...
                >
                  ...
                  {q.label}
                  <motion.span className="dash-quick-btn-arrow" whileHover={{ x: 3, opacity: 1 }}>→</motion.span>
                </motion.button>
              ))}
            </div>
          </motion.div>
      ...
      <div className="dash-tips-list">
        {tips.map((tip, i) => (
          <motion.div
            ...
            style={{ background: levelBg[tip.level], border: `1px solid ${levelColor[tip.level]}18`, borderLeft: `3px solid ${levelColor[tip.level]}` }}
          >
            ...
            <motion.button
              ...
              style={{ background: `${levelColor[tip.level]}18`, border: `1px solid ${levelColor[tip.level]}30`, color: levelColor[tip.level] }}
            >
              {tip.action} →
            </motion.button>
          </motion.div>
...
function QuickStat({ label, value, color, icon }: { label: string; value: string; color: string; icon: string }) {
  return (
    ...
    >
      <motion.div whileHover={{ scale: 1.15, rotate: 5 }} className="dash-quickstat-icon" style={{ background: `${color}15` }}>{icon}</motion.div>
      <span className="dash-quickstat-label">{label}</span>
      <span className="dash-quickstat-value" style={{ color }}>{value}</span>
    </motion.div>
  );
}
...
function FormulaItem({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    ...
    >
      ...
      <div className="dash-formula-value" style={{ color }}>{formatMoney(value)}</div>
    </motion.div>
  );
}
