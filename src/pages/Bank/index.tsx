...
function filterTransactions(
  ...
  if (dateTo)
    result = result.filter((t) => t.date <= dateTo + "T23:59:59")
  return [...result].sort(
    (a, b) =>
...
function parseCsvText(text: string) {
  ...
  const hasDesc =
    header.includes("açıklama") ||
    header.includes("description") ||
    header.includes("aciklama")
  const hasAmount =
  ...
  const parsed = lines
    ...
    .map((line) => {
      ...
      const find = (kw: string[]) => {
        const idx = header
          .split(/[;,]/)
          .findIndex((h) => kw.some((k) => h.trim().toLowerCase().includes(k)))
        return idx >= 0 ? cols[idx]?.trim() : ""
      }
      const rawAmount = find(["tutar", "amount", "miktar"])
      const amount = Math.abs(
        parseFloat(rawAmount.replace(",", ".").replace(/[^0-9.-]/g, "")),
      )
      const rawDate =
        find(["tarih", "date"]) || new Date().toISOString().split("T")[0]
...
export default function Bank({ db, save }: Props) {
  ...
  const handleCsvFile = (file: File) => {
    ...
    reader.onload = (evt) => {
      ...
      if (!parsed) {
        showToast(
          'CSV\'de en az "Açıklama" ve "Tutar" sütunları olmalı!',
          "error",
        )
        return
      }
  ...
  const handleExcelDownload = () => {
    const rows = sorted.map((t) => ({
      ...
      Durum:
        STATUS_LABEL[t.status || "unmatched"]?.label || "",
  ...
  const handleAiMatch = () => {
    const unmatched = db.bankTransactions.filter(
      (t) => t.status === "unmatched" || t.status === "matched",
    )
    ...
    setTimeout(
      () => showToast(`${matchCount} işlem eşlendi!`, "success"),
      100,
    )
  }
