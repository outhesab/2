...
export default function Pelet({ db, save }: Props) {
  ...
  const [supForm, setSupForm] = useState<Partial<PeletSupplier>>({
    name: '',
    phone: '',
    email: '',
    address: '',
  ...
  const saveOrder = () => {
    ...
    if (!qty || !unitPrice) {
      showToast('Miktar ve fiyat girin!', 'error');
      return;
    }
    const nowIso = new Date().toISOString();
    save((prev) =>
    ...
    <div>
      ...
      >
        <div style={{ background: 'var(--bg-card)', borderRadius: 12, padding: 18, border: '1px solid #334155' }}>
          ...
          <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#ff5722', marginTop: 4 }}>
            ₺{((pellet.gramaj / 1000) * pellet.kgFiyat).toFixed(4)}
          </div>
          <div style={{ color: '#475569', fontSize: '0.75rem', marginTop: 2 }}>
            {pellet.gramaj}g/saat × ₺{pellet.kgFiyat}/kg
          </div>
        </div>
        <div style={{ background: 'var(--bg-card)', borderRadius: 12, padding: 18, border: '1px solid #334155' }}>
          ...
          <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#10b981', marginTop: 4 }}>
            {formatMoney(pellet.cuvalKg * pellet.kgFiyat)}
          </div>
          <div style={{ color: '#475569', fontSize: '0.75rem', marginTop: 2 }}>
            {pellet.cuvalKg}kg × ₺{pellet.kgFiyat}
          </div>
        </div>
        <div style={{ background: 'var(--bg-card)', borderRadius: 12, padding: 18, border: '1px solid #334155' }}>
          <div style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>🏭 Tedarikçi Sayısı</div>
          <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#3b82f6', marginTop: 4 }}>
            {db.peletSuppliers.length}
          </div>
        </div>
      </div>
      ...
      >
        <div
          style={{
            background: 'linear-gradient(135deg, #ff572212, #ff572204)',
            borderRadius: 12,
            border: '1px solid #ff572222',
            padding: 14,
          }}
        >
          <div style={{ color: 'var(--text-muted)', fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase' }}>
            🔥 Günlük Tüketim
          </div>
          <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#ff5722', marginTop: 3 }}>
            {((pellet.gramaj / 1000) * pellet.kgFiyat * 10).toFixed(0)} kg
          </div>
          <div style={{ color: '#475569', fontSize: '0.72rem' }}>
            ~10 saat/gün = ₺{((pellet.gramaj / 1000) * pellet.kgFiyat * 10).toFixed(1)}
          </div>
        </div>
        <div
          style={{
            background: 'linear-gradient(135deg, #10b98112, #10b98104)',
            borderRadius: 12,
        ...
        >
          ...
          <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#10b981', marginTop: 3 }}>
            {((pellet.gramaj / 1000) * pellet.kgFiyat * 10 * 26).toFixed(1)} kg
          </div>
          <div style={{ color: '#475569', fontSize: '0.72rem' }}>
            ₺{((pellet.gramaj / 1000) * pellet.kgFiyat * 10 * 26).toFixed(0)}/ay
          </div>
        </div>
        <div
          style={{
            background: 'linear-gradient(135deg, #3b82f612, #3b82f604)',
            borderRadius: 12,
            ...
            {(() => {
              ...
              const totalConsumed =
                (pellet.gramaj / 1000) *
                  10 *
                  26 *
                  ((db.peletOrders.filter((o) => o.status === 'tamamlandi').length || 1) - 1) || 0;
        ...
        <div
          style={{
            background: 'linear-gradient(135deg, #a78bfa12, #a78bfa04)',
            borderRadius: 12,
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        ...
        <button
          onClick={() => {
            setSupForm({ name: '', phone: '', email: '', address: '', note: '', tonPrice: 0 });
            setEditSupId(null);
            setSupModal(true);
          }}
          style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px,1fr))', gap: 14 }}>
          {db.peletSuppliers.length === 0 ? (
            ...
            db.peletSuppliers.map((s) => (
              <div
                key={s.id}
                style={{ background: 'var(--bg-card)', borderRadius: 12, border: '1px solid #334155', padding: 18 }}
              >
                <h4 style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}>🪵 {s.name}</h4>
                {s.phone && (
                  <p style={{ color: 'var(--text-dim)', fontSize: '0.85rem', marginBottom: 4 }}>📞 {s.phone}</p>
                )}
                {s.tonPrice && (
                  <p style={{ color: '#10b981', fontSize: '0.88rem', fontWeight: 700, marginBottom: 4 }}>
                    ₺{s.tonPrice}/ton
                  </p>
                )}
      ...
      {tab === 'orders' && (
        <div
          ...
          style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid #334155', overflowX: 'auto' }}
        >
                ...
                [...db.peletOrders]
                  ...
                  .map((o) => (
                    <tr key={o.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <td
                        data-label="Tarih"
                        style={{ padding: '12px 16px', color: 'var(--text-muted)', fontSize: '0.82rem' }}
                      >
                        {formatDate(o.createdAt)}
                      </td>
                      <td
                        data-label="Tedarikçi"
                        style={{ padding: '12px 16px', color: 'var(--text-primary)', fontWeight: 600 }}
                      >
                        {db.peletSuppliers.find((s) => s.id === o.supplierId)?.name || '-'}
                      </td>
                      <td data-label="Miktar" style={{ padding: '12px 16px', color: 'var(--text-dim)' }}>
                        {o.qty} ton
                      </td>
                      <td data-label="Birim Fiyat" style={{ padding: '12px 16px', color: 'var(--text-dim)' }}>
                        ₺{o.unitPrice}/ton
                      </td>
                      <td data-label="Toplam" style={{ padding: '12px 16px', color: '#10b981', fontWeight: 700 }}>
      ...
      <Modal
        ...
        title={editSupId ? '✏️ Tedarikçi Düzenle' : '➕ Yeni Pelet Tedarikçisi'}
      >
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          <div>
            <label style={lbl}>Ton Fiyatı (₺)</label>
            <input
              type="number"
              inputMode="decimal"
              value={supForm.tonPrice || ''}
              onChange={(e) => setSupForm((f) => ({ ...f, tonPrice: parseFloat(e.target.value) || 0 }))}
              style={inp}
              step={0.01}
            />
          </div>
      ...
      <Modal open={orderModal} onClose={() => setOrderModal(false)} title="📦 Pelet Sipariş Ver">
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <label style={lbl}>Miktar (ton)</label>
              <input
                type="number"
                inputMode="decimal"
                value={orderForm.qty}
            ...
            <div>
              <label style={lbl}>Birim Fiyat (₺/ton)</label>
              <input
                type="number"
                inputMode="decimal"
          ...
          {orderForm.qty && orderForm.unitPrice && (
            <div style={{ background: '#0f172a', borderRadius: 8, padding: '10px 14px' }}>
              ...
              <strong style={{ color: '#10b981' }}>
                {formatMoney(parseFloat(orderForm.qty) * parseFloat(orderForm.unitPrice))}
              </strong>
            </div>
          )}
        </div>
