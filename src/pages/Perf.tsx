...
export default function Perf() {
  ...
  useEffect(() => {
    const logs = logger
      .getLogs({ cat: 'perf', limit: 200 })
      .concat(logger.getLogs({ cat: 'db', limit: 200 }), logger.getLogs({ cat: 'firebase', limit: 100 }));
    const rows = parsePerfEntries(logs);
    setPerfLogs(rows);

    const unsub = logger.subscribe((entry) => {
      if (entry.ms !== undefined && (entry.cat === 'perf' || entry.cat === 'db' || entry.cat === 'firebase')) {
        setPerfLogs((prev) => [{ label: entry.msg, ms: entry.ms!, ts: entry.ts }, ...prev].slice(0, 500));
      }
    });
    return unsub;
  }, []);

  const filtered = filter ? perfLogs.filter((r) => r.label.toLowerCase().includes(filter.toLowerCase())) : perfLogs;
        ...
        <tbody>
          {filtered.map((row, i) => (
            <tr key={i} style={{ borderBottom: '1px solid #1e293b' }}>
              <td
                style={{ padding: '4px 12px', color: row.ms > 100 ? '#f87171' : row.ms > 30 ? '#fbbf24' : '#94a3b8' }}
              >
              ...
              <td style={{ padding: '4px 12px', color: '#64748b', fontSize: 12 }}>{row.ts.slice(11, 23)}</td>
            </tr>
          ))}
        </tbody>
      </table>
