...
import { getHealthScore, getIntegritySummary, runIntegrityCheck } from '@/lib/dataIntegrityChecker';
...
export default function Monitor({ db, save }: Props) {
        ...
        .flatMap((r) => {
          ...
          if (r.type === 'stok_sifir') {
            ...
            const bugunCiro = db.sales
              .filter((s) => !s.deleted && s.status === 'tamamlandi' && s.createdAt.slice(0, 10) === todayStr)
  ...
  return (
    <div>
      <MonitorOverview healthScore={healthScore} summary={summary} alertCount={alerts.length} />
      ...
      {tab === 'rules' && <MonitorRules rules={db.monitorRules} save={save} />}
      ...
      {tab === 'errors' && (
        <>
          ...
          {errorLogs.length === 0 ? (
            <EmptyState icon={Bug} title="Hata kaydı yok!" description="Uygulama hatasız çalışıyor." />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {errorLogs.map((log) => (
                <div
