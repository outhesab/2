...
export default function MonitorAuditLog({ auditLog }: Props) {
  ...
  const filteredAuditLog = auditLog.filter((e) => auditStatusFilter === 'all' || e.status === auditStatusFilter);
      ...
      {filteredAuditLog.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {filteredAuditLog.slice(0, 200).map((entry) => {
            const statusColor =
              entry.status === 'blocked' ? '#ef4444' : entry.status === 'warned' ? '#f59e0b' : '#10b981';
            const statusLabel =
              entry.status === 'blocked' ? '🚫 Engellendi' : entry.status === 'warned' ? '⚠️ Uyarı' : '✅ Uygulandı';
            const isExpanded = expandedAuditId === entry.id;

            const prevKeys =
              entry.prevValue && typeof entry.prevValue === 'object' ? Object.keys(entry.prevValue as object) : [];
            const nextKeys =
              entry.nextValue && typeof entry.nextValue === 'object' ? Object.keys(entry.nextValue as object) : [];
                  ...
                  >
                    {entry.violations.map((v, i) => (
                      ...
                      >
                        <span
                          style={{
                            color: v.severity === 'block' ? '#ef4444' : '#f59e0b',
                            fontSize: '0.8rem',
                            fontWeight: 700,
                            whiteSpace: 'nowrap',
                          }}
