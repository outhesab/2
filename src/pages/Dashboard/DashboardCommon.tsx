...
export function QuickStat({
  ...
  return (
    <motion.div whileHover={{ scale: 1.02, background: `${color}10` }} className="dash-quick-stat">
      <span className="dash-quick-stat-icon" style={{ color }}>
        {icon}
      </span>
      <div className="dash-quick-stat-content">
        <div className="dash-quick-stat-label">{label}</div>
        <div className="dash-quick-stat-value" style={{ color }}>
          {value}
        </div>
      </div>
    </motion.div>
...
export function FormulaItem({ label, value, color }: { label: string; value: number; color: string }) {
  return (
