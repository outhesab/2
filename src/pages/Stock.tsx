...
export default function Stock({ db, save }: Props) {
  ...
  const [form, setForm] = useState({
    productId: '',
    type: 'giris' as 'giris' | 'cikis' | 'duzeltme',
    amount: '',
    note: '',
  });
  ...
  const sortedMovements = [...movements].sort(
    (a, b) => new Date(b.date || b.id).getTime() - new Date(a.date || a.id).getTime(),
  );
  ...
  const handleAdjust = () => {
    ...
    if (!amount || amount <= 0) {
      showToast('Geçerli miktar girin!', 'error');
      return;
    }
    ...
    const after =
      form.type === 'giris' ? before + amount : form.type === 'cikis' ? Math.max(0, before - amount) : amount;
    ...
    save((prev) => ({
      ...
      stockMovements: [
        ...prev.stockMovements,
        {
          ...
          type: form.type,
          amount: form.type === 'duzeltme' ? after - before : amount,
          before,
          after,
          note: form.note,
          date: nowIso,
  ...
  const abcData = useMemo(() => {
    ...
    db.sales
      ...
      .forEach((s) => {
        ...
        if (!productRev[s.productId])
          productRev[s.productId] = {
            name: s.productName,
    ...
    <div>
      ...
      <div className="flex gap-3 mb-4 items-center flex-wrap">
        <Button
          ...
          className="bg-[#ff5722] hover:bg-[#e64a19] text-white font-bold rounded-xl px-5"
        >
          ⚙️ Stok Ayarla
        </Button>
        <Button
              ...
              <tbody>
                {sortedProducts.length === 0 ? (
                  ...
                  sortedProducts.map((p) => {
                    const stockStatus =
                      p.stock === 0
                        ? { color: '#ef4444', label: '🔴 Bitti', bg: 'bg-red-500/10' }
                        : p.stock <= p.minStock
                          ? { color: '#f59e0b', label: '⚠️ Az', bg: 'bg-amber-500/10' }
                          : { color: '#10b981', label: '✓ Normal', bg: 'bg-emerald-500/10' };
                    ...
                    return (
                      ...
                      >
                        ...
                        <td data-label="Kategori" className="p-3 text-slate-400 text-sm">
                          {catName}
                        </td>
                        <td data-label="Stok" className="p-3 font-bold text-base" style={{ color: stockStatus.color }}>
                          {p.stock}
                        </td>
                        <td data-label="Min.Stok" className="p-3 text-muted-foreground">
                          {p.minStock}
                        </td>
                        <td data-label="Durum" className="p-3">
                          <Badge
                            variant="outline"
                            className={`font-semibold text-xs ${stockStatus.bg} ${stockStatus.color.replace('#', 'text-')} border-transparent`}
                          >
                            {stockStatus.label}
                          </Badge>
                        </td>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            {(['A', 'B', 'C'] as const).map((cls) => {
              ...
              return (
                <div
                  key={cls}
                  className={`rounded-xl p-4 border transition-all hover:scale-[1.02] ${colors[cls] === '#10b981' ? 'bg-emerald-500/10 border-emerald-500/25' : colors[cls] === '#3b82f6' ? 'bg-blue-500/10 border-blue-500/25' : 'bg-slate-500/10 border-slate-500/25'}`}
                >
                  <div style={{ color: colors[cls] }} className="text-2xl font-black">
                    {d.count} ürün
                  </div>
                    ...
                    return (
                      ...
                      >
                        <td data-label="Sınıf" className="p-3">
                          <Badge
                            variant="outline"
                            className={`font-bold text-xs ${v.class === 'A' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : v.class === 'B' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-slate-500/10 text-slate-400 border-slate-500/20'}`}
                          >
                            {v.class}
                          </Badge>
                        </td>
                        ...
                        <td data-label="Ciro" className="p-3 text-emerald-500 font-bold">
                          ₺{(v.revenue / 1000).toFixed(1)}K
                        </td>
                        <td data-label="Ciro %" className="p-3 text-slate-400">
                          %{v.revenuePct.toFixed(1)}
                        </td>
                        <td data-label="Küm.%" className="p-3">
                          <div className="h-1.5 rounded-full bg-slate-800 overflow-hidden max-w-[80px]">
                            <div
                              style={{ width: `${v.cumulPct}%`, background: clsColor }}
                              className="h-full rounded-full"
                            />
                          </div>
                        </td>
                        <td data-label="Adet" className="p-3 text-slate-400">
                          {v.qty}
                        </td>
                        <td
                          data-label="Kâr"
                          className={`p-3 font-semibold ${v.profit >= 0 ? 'text-amber-500' : 'text-red-500'}`}
                        >
                ...
                {deadStock.length === 0 ? (
                  ...
                  deadStock.map((p) => (
                    <tr key={p.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td data-label="Stok" className="p-3 text-red-500 font-bold">
                        {p.stock}
                      </td>
                      <td data-label="Maliyet" className="p-3 text-slate-400">
                        {formatMoney(p.cost)}
                      </td>
                      <td data-label="Değer" className="p-3 text-amber-500 font-bold">
                        {formatMoney(p.cost * p.stock)}
                      </td>
                      <td data-label="Son Hareket" className="p-3 text-muted-foreground text-xs">
                        {formatDate(p.lastMovement)}
                      </td>
                      <td data-label="Gün" className="p-3 text-red-500 font-semibold">
                        {p.daysSince}g
                      </td>
                    </tr>
                  ))
        ...
        <>
          <div className="flex gap-3 mb-4 flex-wrap">
            ...
            <select
              ...
              onChange={(e) => {
                setHistTypeFilter(e.target.value);
                setHistPage(1);
              }}
                ...
                {pagedMovements.length === 0 ? (
                  ...
                  pagedMovements.map((m) => {
                    const typeMap: Record<string, { label: string; color: string }> = {
                      giris: { label: '📥 Giriş', color: '#10b981' },
                      cikis: { label: '📤 Çıkış', color: '#ef4444' },
                      satis: { label: '🛒 Satış', color: '#3b82f6' },
                      iade: { label: '↩️ İade', color: '#8b5cf6' },
                      duzeltme: { label: '⚙️ Düzeltme', color: '#f59e0b' },
                      siparis: { label: '📦 Sipariş', color: '#8b5cf6' },
                    };
                    ...
                    return (
                      <tr key={m.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td data-label="İşlem" className="p-3">
                          <span style={{ color: t.color }} className="font-semibold text-xs">
                            {t.label}
                          </span>
                        </td>
                        <td
                          data-label="Miktar"
                          className={`p-3 font-bold ${m.amount >= 0 ? 'text-emerald-500' : 'text-red-500'}`}
                        >
                          {m.amount > 0 ? '+' : ''}
                          {m.amount}
                        </td>
                        <td data-label="Önceki" className="p-3 text-slate-400">
                          {m.before}
                        </td>
                        <td data-label="Sonraki" className="p-3 text-foreground font-bold">
                          {m.after}
                        </td>
                        <td data-label="Not" className="p-3 text-muted-foreground text-xs">
                          {m.note || '-'}
                        </td>
                      </tr>
                    );
          ...
          {totalHistPages > 1 && (
            <div className="flex justify-center items-center gap-3 mt-4">
              ...
              <span className="text-muted-foreground text-xs font-medium">
                Sayfa {histPage} / {totalHistPages}{' '}
                <span className="opacity-50 ml-1">(toplam {sortedMovements.length} kayıt)</span>
              </span>
              <button
                onClick={() => {
      ...
      <Modal open={adjustModal} onClose={() => setAdjustModal(false)} title="⚙️ Stok Ayarla">
        <div className="grid gap-4">
          ...
          <div>
            <label className="block mb-2 text-slate-400 text-sm font-medium">
              {form.type === 'duzeltme' ? 'Yeni Stok Miktarı' : 'Miktar'}
            </label>
            <input
              type="number"
              inputMode="decimal"
        ...
        <div className="flex gap-3 mt-6">
          <button
            ...
            className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white py-2.5 rounded-xl font-bold text-sm transition-all active:scale-95"
          >
            💾 Kaydet
          </button>
          <button
...
function StatCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div
      style={{ background: 'var(--bg-card)', borderRadius: 12, padding: '16px 18px', border: `1px solid ${color}22` }}
    >
