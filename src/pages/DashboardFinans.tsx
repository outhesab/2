...
function GoldCard({
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.01, borderColor: `${accent}40`, boxShadow: `0 8px 32px ${accent}08` }}
      className="df-card"
      style={{
    ...
    >
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: `linear-gradient(135deg, ${accent}04, transparent 60%)`,
          pointerEvents: 'none',
        }}
      />
      <div
      ...
      >
        <h3 style={{ color: 'var(--text-primary)', fontSize: '1rem', fontWeight: 600, letterSpacing: '0.02em' }}>
          {title}
        </h3>
        {extra}
      </div>
...
function StatNumber({
  ...
  return (
    <div style={{ textAlign: 'center', padding: '10px 6px' }}>
      ...
      <div style={{ color, fontSize: '1.3rem', fontWeight: 700, fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
        {value}
      </div>
...
export default function DashboardFinans({ db, onTabChange }: Props) {
  ...
  const finansData = useMemo(() => {
    ...
    const daily: Record<
      string,
      { gelir: number; gider: number; nakit: number; kart: number; havale: number; cariPay: number }
    > = {};
    ...
    const banka = activeKasa
      ...
      .reduce((s, k) => s + (k.type === 'gelir' ? k.amount : -k.amount), 0);
    const posZiraat = activeKasa
      .filter((k) => k.kasa === 'pos_ziraat')
      .reduce((s, k) => s + (k.type === 'gelir' ? k.amount : -k.amount), 0);
    const posIs = activeKasa
      .filter((k) => k.kasa === 'pos_is')
      .reduce((s, k) => s + (k.type === 'gelir' ? k.amount : -k.amount), 0);
    const posYk = activeKasa
      .filter((k) => k.kasa === 'pos_yk')
      .reduce((s, k) => s + (k.type === 'gelir' ? k.amount : -k.amount), 0);
    const posToplam = posZiraat + posIs + posYk;
    const musteriAlacak = db.cari
      .filter((c) => !c.deleted && c.type === 'musteri' && c.balance > 0)
      .reduce((s, c) => s + c.balance, 0);
    const tedarikciBorç = db.cari
    ...
    const buAyCiro = db.sales
      .filter((s) => !s.deleted && s.status === 'tamamlandi' && new Date(s.createdAt) >= ayBası)
      .reduce((s, x) => s + x.total, 0);
    const gecenAyCiro = db.sales
      .filter(
        (s) =>
          !s.deleted &&
          s.status === 'tamamlandi' &&
          new Date(s.createdAt) >= oncekiAyBası &&
          new Date(s.createdAt) < ayBası,
      )
      .reduce((s, x) => s + x.total, 0);
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>
            v{getAppVersion()} · DB: {db._version || 0} · {formatDate(new Date().toISOString())}
          </p>
        </div>
          ...
          {['14d', '30d'].map((f) => (
            <motion.button
              ...
              style={{
                padding: '6px 16px',
                borderRadius: 8,
                border: `1px solid ${filter === f ? GOLD : `${GOLD}30`}`,
                background: filter === f ? `${GOLD}20` : 'transparent',
                color: filter === f ? GOLD : 'var(--text-secondary)',
                cursor: 'pointer',
                fontSize: '0.8rem',
                fontWeight: 500,
              }}
      ...
      <motion.div
        variants={cardVariants}
        style={{
          background: `linear-gradient(135deg, ${CARD_BG}, var(--bg-elevated))`,
          borderRadius: 14,
          border: `1px solid ${GOLD}18`,
          borderTop: `3px solid ${healthScore.color}`,
          padding: '20px 22px',
          marginBottom: 16,
          display: 'flex',
          alignItems: 'center',
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: 'var(--text-primary)', fontSize: '0.95rem', fontWeight: 600 }}>
            Finansal Sağlık: {healthScore.label}
          </div>
        ...
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          <StatNumber
            label="Bu Ay Ciro"
            value={formatMoney(finansData.buAyCiro)}
            color={finansData.ciroFarkPct >= 0 ? 'var(--color-success)' : 'var(--color-danger)'}
            sub={`%${finansData.ciroFarkPct.toFixed(1)} geçen aya göre`}
          />
          <StatNumber
            label="Net Sermaye"
            value={formatMoney(finansData.netSermaye)}
            color={finansData.netSermaye >= 0 ? GOLD : 'var(--color-danger)'}
          />
          ...
          {finansData.chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={finansData.chartData} margin={{ top: 8, right: 8, bottom: 4, left: -8 }}>
                ...
                />
                <Tooltip
                  formatter={(v: number, n: string) => [formatMoney(v), n === 'gelir' ? 'Gelir' : 'Gider']}
      ...
      {(() => {
        ...
        const aging = {
          '0-30': [] as typeof musteri,
          '31-60': [] as typeof musteri,
          '61-90': [] as typeof musteri,
          '90+': [] as typeof musteri,
        };
        musteri.forEach((c) => {
        ...
        return (
          <div style={{ marginBottom: 14 }}>
            <GoldCard
              ...
              subtitle="Son işlem tarihine göre müşteri alacakları"
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>
                        {range} gün ({customers.length} müşteri)
                      </div>
                      <div style={{ display: 'flex', gap: 4, marginTop: 6, height: 4 }}>
                        {['0-30', '31-60', '61-90', '90+'].map((r) => {
                          const pct =
                            musteri.length > 0
                              ? ((aging as Record<string, typeof musteri>)[r].length / musteri.length) * 100
                              : 0;
                          return (
                            <div
                              key={r}
                              style={{
                                ...
                                height: 4,
                                borderRadius: 2,
                                background: agingColors[r],
                                opacity: r === range ? 1 : 0.3,
                              }}
                            />
                          );
                        })}
                  ...
                  <div className="responsive-table-wrap" style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
                      ...
                      <tbody>
                        {musteri
                          .sort((a, b) => {
                            const aDays = a.lastTransaction
                              ? Math.floor((now - new Date(a.lastTransaction).getTime()) / dayMs)
                              : 999;
                            const bDays = b.lastTransaction
                              ? Math.floor((now - new Date(b.lastTransaction).getTime()) / dayMs)
                              : 999;
                            return bDays - aDays;
                          })
                          .slice(0, 10)
                          .map((c) => {
                            const days = c.lastTransaction
                              ? Math.floor((now - new Date(c.lastTransaction).getTime()) / dayMs)
                              : 999;
                            ...
                            return (
                              <tr key={c.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                ...
                                <td style={{ padding: '6px 8px', color: 'var(--color-success)', fontWeight: 700 }}>
                                ...
                                <td style={{ padding: '6px 8px' }}>
                                  <span
                                    style={{
                                      background: `${agingColors[range]}20`,
                                      color: agingColors[range],
                                      borderRadius: 4,
                                      padding: '2px 6px',
                                      fontSize: '0.68rem',
                                      fontWeight: 600,
      ...
      >
        <GoldCard title="Nakit" accent="#06b6d4">
          <StatNumber label="Bakiye" value={formatMoney(finansData.nakit)} color="#06b6d4" />
        </GoldCard>
        <GoldCard title="Banka" accent="#6366f1">
          <StatNumber label="Bakiye" value={formatMoney(finansData.banka)} color="#6366f1" />
        </GoldCard>
        <GoldCard title="POS Ziraat" accent="var(--color-warning)">
          <StatNumber label="Bakiye" value={formatMoney(finansData.posZiraat)} color="var(--color-warning)" />
        </GoldCard>
        <GoldCard title="POS İş Bankası" accent="var(--color-info)">
          <StatNumber label="Bakiye" value={formatMoney(finansData.posIs)} color="var(--color-info)" />
        </GoldCard>
        <GoldCard title="POS YK" accent="var(--color-accent)">
          <StatNumber label="Bakiye" value={formatMoney(finansData.posYk)} color="var(--color-accent)" />
        </GoldCard>
        <GoldCard title="Müşteri Alacak" accent="var(--color-success)">
          <StatNumber label="Toplam" value={formatMoney(finansData.musteriAlacak)} color="var(--color-success)" />
        </GoldCard>
        <GoldCard title="Tedarikçi Borç" accent="var(--color-danger)">
          <StatNumber label="Toplam" value={formatMoney(finansData.tedarikciBorç)} color="var(--color-danger)" />
        </GoldCard>
      </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {[
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.04, background: `${b.color}20` }}
              whileTap={{ scale: 0.96 }}
              style={{
                padding: '10px 18px',
                borderRadius: 10,
