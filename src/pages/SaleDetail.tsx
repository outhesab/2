...
export default function SaleDetail({ db }: Props) {
  ...
  const cari = sale.cariId ? db.cari.find((item) => item.id === sale.cariId) : undefined;
  const invoices = db.invoices.filter((invoice) => !invoice.deleted && invoice.saleId === sale.id);
  const kasaEntries = db.kasa.filter(
    (entry) =>
      !entry.deleted &&
      (entry.relatedId === sale.id || entry.description?.toLowerCase().includes(sale.productName.toLowerCase())),
  );
  const productIds = new Set((sale.items || []).map((item) => item.productId).concat(sale.productId || []));
  ...
  const auditLog = (db._auditLog || []).filter((entry) => auditMentionsSale(entry, sale.id));
  ...
  return (
    <div>
      ...
      <section style={{ ...card, marginTop: 16 }}>
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(145px,1fr))', gap: 10, marginTop: 18 }}
        >
...
function StatusBadge({ sale }: { sale: Sale }) {
  return (
    ...
    >
      {sale.status === 'tamamlandi' ? 'Tamamlandı' : sale.status === 'iade' ? 'İade' : 'İptal'}
    </span>
  );
}
