...
function GlassCard({
  ...
  return (
    <motion.div
      ...
      whileHover={{
        scale: 1.01,
        boxShadow: `0 8px 40px ${accent || '#7c3aed'}15`,
        borderColor: `${accent || '#7c3aed'}20`,
      }}
      style={{
    ...
    >
      ...
      >
        <h3 style={{ color: 'var(--text-primary)', fontSize: '0.9rem', fontWeight: 500, letterSpacing: '0.01em' }}>
          {title}
        </h3>
        {extra}
      </div>
...
export default function DashboardStrateji({ db, onTabChange }: Props) {
  ...
  const insights = useMemo(() => {
    ...
    const gecenAy = db.sales.filter(
      (s) =>
        !s.deleted &&
        s.status === 'tamamlandi' &&
        new Date(s.createdAt) >= oncekiAyBası &&
        new Date(s.createdAt) <= oncekiAySonu,
    );
    ...
    const gunlukGider =
      db.kasa
        .filter((k) => !k.deleted && k.type === 'gider' && k.createdAt >= son30Gun)
        .reduce((s, k) => s + k.amount, 0) / 30;
    const nakit = db.kasa
    ...
    return {
      ...
      gecenAyCiro,
      changePct,
      topCat: topCat ? { name: topCat[0], value: topCat[1] } : null,
      cashRunway,
      stockAlert,
      gunlukGider,
    };
  ...
  const anomalies = useMemo(() => {
    ...
    return completed
      .filter((s) => Math.abs(s.total - mean) > 2.5 * std)
      .sort((a, b) => Math.abs(b.total - mean) - Math.abs(a.total - mean))
      .slice(0, 5)
      .map((s) => ({
        ...s,
        severity:
          Math.abs(s.total - mean) > 3.5 * std
            ? ('HIGH' as const)
            : Math.abs(s.total - mean) > 3 * std
              ? ('MEDIUM' as const)
              : ('LOW' as const),
      }));
  }, [db.sales]);
    ...
    >
      <motion.div variants={cardVariants} style={{ marginBottom: 16, textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 300 }}>
          AI Destekli İçgörüler ve Tahminler
        </p>
      </motion.div>
      ...
      >
          ...
          {!loading && (
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 300, color: 'var(--text-primary)' }}>
                {formatMoney(insights.buAyCiro)}
              </div>
              <div
                style={{
                  fontSize: '0.82rem',
                  color: insights.changePct >= 0 ? 'var(--color-success)' : 'var(--color-danger)',
                  marginTop: 4,
                }}
              >
              ...
              <div style={{ color: 'var(--text-muted)', fontSize: '0.72rem', marginTop: 2 }}>
                Geçen ay: {formatMoney(insights.gecenAyCiro)}
              </div>
            </div>
          )}
        </GlassCard>
        ...
        <GlassCard title="En İyi Kategori" accent="var(--color-info)" shimmer={loading}>
          {!loading && (
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              <div style={{ fontSize: '1.2rem', fontWeight: 400, color: 'var(--text-primary)', marginBottom: 4 }}>
                {insights.topCat?.name || '-'}
              </div>
              <div style={{ fontSize: '1.3rem', fontWeight: 300, color: 'var(--color-info)' }}>
                {insights.topCat ? formatMoney(insights.topCat.value) : 'Veri yok'}
              </div>
            </div>
          )}
        </GlassCard>
          ...
          {!loading && (
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              ...
              <div
                style={{
                  color:
                    insights.cashRunway < 30
                      ? 'var(--color-danger)'
                      : insights.cashRunway < 60
                        ? 'var(--color-warning)'
                        : 'var(--color-success)',
                  fontSize: '0.72rem',
                }}
            ...
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              <div
                style={{
                  fontSize: '1.5rem',
                  fontWeight: 300,
                  color: insights.stockAlert > 0 ? 'var(--color-danger)' : 'var(--color-success)',
                }}
              >
                {insights.stockAlert}
              </div>
      ...
      <GlassCard
        title="Ciro Tahmini"
        subtitle="Son 30 gün gerçek + 7 gün tahmin (basit projeksiyon)"
        accent="linear-gradient(90deg, #7c3aed, var(--color-info))"
      >
        {loading ? (
            ...
            <AreaChart data={forecastData} margin={{ top: 8, right: 8, bottom: 4, left: -8 }}>
              ...
              <Tooltip
                formatter={(v: number, n: string) => [
                  formatMoney(v),
                  n === 'actual' ? 'Gerçek' : n === 'forecast' ? 'Tahmin' : n,
                ]}
        ...
        >
          <span>
            <span style={{ color: '#7c3aed' }}>━</span> Gerçek
          </span>
          <span>
            <span style={{ color: 'var(--color-info)', borderBottom: '2px dashed var(--color-info)' }}>━</span> Tahmin
          </span>
        </div>
      </GlassCard>
      ...
      {/* Sezonsallık Grafiği */}
      <GlassCard
        title="Sezonsallık Analizi"
        subtitle="Aylık ortalama ciro dağılımı ve 3 aylık nakit akışı tahmini"
            ...
            <div>
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={seasonalityData.monthlyData} margin={{ top: 4, right: 4, bottom: 4, left: -16 }}>
                  ...
                  />
                  <Tooltip
                    formatter={(v: number, n: string) => [formatMoney(v), n === 'avg' ? 'Ortalama' : n]}
                    contentStyle={{
                      background: 'rgba(10,10,20,0.95)',
                      border: '1px solid rgba(255,255,255,0.06)',
                      borderRadius: 8,
                  ...
                  <Bar dataKey="avg" radius={[3, 3, 0, 0]} maxBarSize={20}>
                    {seasonalityData.monthlyData.map((entry, idx) => (
                      <Cell
                        key={idx}
                        fill={
                          entry.coefficient > 1.2
                            ? 'var(--color-success)'
                            : entry.coefficient > 0.8
                              ? '#8b5cf6'
                              : 'var(--text-muted)'
                        }
                      />
              ...
              >
                <span>
                  <span style={{ color: 'var(--color-success)' }}>●</span> Yüksek
                </span>
                <span>
                  <span style={{ color: '#8b5cf6' }}>●</span> Normal
                </span>
                <span>
                  <span style={{ color: 'var(--text-muted)' }}>●</span> Düşük
                </span>
              </div>
            </div>
            <div>
                ...
                >
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <span style={{ color: 'var(--color-danger)', fontSize: '0.75rem', fontWeight: 600 }}>
                      ₺{(c.gider / 1000).toFixed(0)}K
                    </span>
                    <div
                      style={{
                        color: c.net >= 0 ? 'var(--color-success)' : 'var(--color-danger)',
                        fontSize: '0.82rem',
                        fontWeight: 700,
                      }}
                    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 14 }}>
        <GlassCard
          ...
          subtitle={`${anomalies.length} anomali tespit edildi`}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {anomalies.map((a, i) => (
                  ...
                  >
                    ...
                    <span
                      style={{ color: 'var(--text-primary)', fontFamily: "'Courier New',monospace", marginRight: 10 }}
                    >
                      {formatMoney(a.total)}
                    </span>
                    <span
                      style={{
                        ...
                        fontWeight: 600,
                        background:
                          a.severity === 'HIGH' ? '#ef444420' : a.severity === 'MEDIUM' ? '#f59e0b20' : '#3b82f620',
                        color:
                          a.severity === 'HIGH'
                            ? 'var(--color-danger)'
                            : a.severity === 'MEDIUM'
        ...
        <GlassCard
          ...
          subtitle="Veriye dayalı simülasyonlar"
          ...
          {!loading && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                {
                  icon: '💳',
                  label: 'Müşteriler %5 erken ödeme yapsa',
                  value: formatMoney(whatIf.earlyPaymentBenefit),
                  detail: `${db.cari.filter((c) => !c.deleted && c.type === 'musteri').length} müşteri baz alındı`,
                  color: 'var(--color-success)',
                },
                {
                  icon: '📈',
                  label: 'Ortalama fiyat %10 artsa',
                  value: `+${formatMoney(whatIf.priceBoost)}/ay`,
                  detail: `Geçen ay ciro: ${formatMoney(whatIf.lastMonthRevenue)}`,
                  color: 'var(--color-info)',
                },
                {
                  icon: '🔄',
                  label: 'Stok devir hızı %20 artarsa',
                  value: `-${formatMoney(whatIf.turnoverSaving)}`,
                  detail: 'Maliyet avantajı',
                ...
                >
                  ...
                  <div style={{ color: 'var(--text-dim)', fontSize: '0.65rem', textAlign: 'right', maxWidth: 120 }}>
                    {item.detail}
                  </div>
                </motion.div>
              ))}
