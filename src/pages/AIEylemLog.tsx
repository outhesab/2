...
export default function AIEylemLog({ db, undo }: Props) {
  ...
  const handleUndo = (entry: AIActionLogEntry) => {
    showConfirm(
      ...
      () => {
        const ok = undo();
        showToast(ok ? 'Son değişiklik geri alındı.' : 'Geri alınacak işlem bulunamadı.', ok ? 'success' : 'error');
      },
    );
  };
  ...
  return (
    <div>
      <section style={card}>
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(145px,1fr))', gap: 10, marginTop: 16 }}
        >
      ...
      <section style={{ ...card, marginTop: 16 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <select
            value={modelFilter}
            onChange={(event) => setModelFilter(event.target.value as typeof modelFilter)}
            style={input}
          >
              ...
              <article key={entry.id} style={row}>
                <div style={{ minWidth: 0 }}>
                  ...
                  {entry.affectedIds && entry.affectedIds.length > 0 && (
                    <div style={{ ...muted, marginTop: 4 }}>Etkilenen kayıtlar: {entry.affectedIds.join(', ')}</div>
                  )}
                  {entry.error && (
                    <div style={{ color: '#f87171', fontSize: '0.82rem', marginTop: 6 }}>{entry.error}</div>
                  )}
