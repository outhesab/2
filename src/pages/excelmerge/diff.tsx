...
import { AlertTriangle, Filter, GitCompare, Minus, Minus as MinusIcon, Pencil, Plus } from 'lucide-react';
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          {commonSheets.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {allHeaders.length > 0 && (
                <div className="space-y-1">
                  <label className="text-sm font-medium text-foreground">Anahtar Sutun (opsiyonel)</label>
      ...
      {diffResult && (
        <>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {filteredRows.map((row) => (
                  <tr key={row.rowIndex} className={`border-b border-border last:border-0 ${statusColor[row.status]}`}>
                      ...
                      return (
                        ...
                        >
                          {isChanged ? (
                            ...
                            String(row.status === 'removed' ? (oldVal ?? '') : (newVal ?? oldVal ?? ''))
                          )}
                        </td>
                      );
                    })}
          ...
          {filteredRows.length > 0 && (
            <p className="text-xs text-muted-foreground text-right">{filteredRows.length} satir gosteriliyor</p>
          )}
        </>
      )}
    </div>
