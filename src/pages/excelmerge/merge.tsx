    ...
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Birden fazla dosyadan veri birlestirin. Join tipi, cakisma stratejisi ve veri temizleme seceneklerini
          yapilandirin.
        </p>
      </div>
        ...
        <CardContent>
          ...
          {selectedFileIds.length > 0 && (
            <p className="text-xs text-muted-foreground mt-3">
              ...
              {selectedFiles
                .reduce((acc, f) => acc + f.sheets.reduce((a, s) => a + s.rows.length, 0), 0)
          ...
          <Card className="border border-card-border">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                {(Object.entries(joinTypeLabels) as [JoinType, (typeof joinTypeLabels)[JoinType]][]).map(
                  ([key, meta]) => (
                    <button
                      key={key}
                      onClick={() => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <Select
                    value={strategy}
                    onValueChange={(v) => {
                      setStrategy(v as 'latest' | 'first' | 'union' | 'intersection');
                      setMerged(null);
                    }}
            ...
            <CardContent className="p-0">
                      ...
                      return (
                        <tr key={i} className="border-b border-border last:border-0 hover:bg-muted/20">
                          ...
                          <td className="px-3 py-2">
                            {src && (
                              <span
                                className="text-xs text-muted-foreground truncate max-w-24 block"
                                title={src.fileName}
                              >
              ...
              {merged.rows.length > 50 && (
                <p className="text-xs text-muted-foreground text-center py-3 border-t border-border">
                  ... ve {(merged.rows.length - 50).toLocaleString('tr-TR')} satir daha. Tamami icin Excel Indir'e
                  basin.
                </p>
              )}
            </CardContent>
