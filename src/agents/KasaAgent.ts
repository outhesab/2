...
export class KasaAgent extends DomainAgent {
  ...
  async islemYap<P = any, R = any>(talep: AgentRequest<P>): Promise<AgentResponse<R>> {
    ...
    if (talep.action === 'kasa_gelir' || talep.action === 'kasa_gider') {
      ...
      if (!v.success) return { ok: false, error: `Kasa İşlemi Hatası: ${v.error.errors.map(e => e.message).join(', ')}` } as AgentResponse<R>;
    }

    const kasaId = (p as any).kasa as string | undefined;
    if (kasaId) {
