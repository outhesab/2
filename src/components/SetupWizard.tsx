...
async function savePassToFirebase(hash: string): Promise<void> {
  try {
    await fetch(FIREBASE_AUTH_URL, {
      ...
      body: JSON.stringify({
        fields: { hash: { stringValue: hash }, updatedAt: { stringValue: new Date().toISOString() } },
      }),
      signal: AbortSignal.timeout(8000),
    });
    // Oturum cache'ine de yaz
    try {
      sessionStorage.setItem('sobaYonetim_hc', hash);
    } catch {
      logger.warn('setup', 'Oturum önbelleğine yazılamadı'); /* ignore */
    }
  } catch {
    logger.warn('setup', "Firebase'e şifre kaydedilemedi"); /* Firebase hata — giriş ekranı tekrar yükleyecek */
  }
}
...
export function getSetupData(): SetupResult | null {
  try {
    ...
    logger.warn('setup', 'Kurulum verisi okunamadı');
    return null;
  }
}
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 12,
        cursor: 'pointer',
        border: `1px solid ${enabled ? 'rgba(255,87,34,0.4)' : 'rgba(255,255,255,0.07)'}`,
        background: enabled ? 'rgba(255,87,34,0.08)' : 'rgba(255,255,255,0.02)',
        transition: 'all 0.2s',
      }}
    >
      ...
      <div
        style={{
          ...
          height: 20,
          borderRadius: '50%',
          background: enabled ? '#ff5722' : 'rgba(255,255,255,0.1)',
          display: 'flex',
          alignItems: 'center',
...
function createFieldArrayActions<T>(setItems: React.Dispatch<React.SetStateAction<T[]>>, factory: () => T) {
  return {
...
export default function SetupWizard({ onComplete }: Props) {
  ...
  const validate = (): boolean => {
    ...
    if (step === 2) {
      if (!kasalar.some((k) => k.enabled)) {
        setErr('En az bir kasa seçin!');
        return false;
      }
    }
    if (step === 3) {
      if (!kategoriler.some((k) => k.enabled)) {
        setErr('En az bir kategori seçin!');
        return false;
      }
    }
    if (step === 4) {
      ...
      for (const u of urunler) {
        if (u.name && (!u.price || !u.cost)) {
          setErr(`"${u.name}" için alış ve satış fiyatı girin!`);
          return false;
        }
      }
    }
    if (step === 5) {
      for (let i = 0; i < ortaklar.length; i++) {
        if (!ortaklar[i].name.trim()) {
          setErr(`${i + 1}. ortak için ad gerekli!`);
          return false;
        }
      }
    }
    if (step === 6) {
      ...
      if (pass.length > 0 && pass.length < 4) {
        setErr('Parola en az 4 karakter olmalı!');
        return false;
      }
    ...
    >
      ...
      >
          ...
          {STEPS.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 7,
                borderRadius: 4,
                background: i < step ? '#ff5722' : i === step ? '#ff9800' : 'rgba(255,255,255,0.1)',
                transition: 'all 0.3s',
              }}
            />
          ))}
        ...
        <div style={{ minHeight: 180 }}>
          ...
          {step === 1 && (
            <StepCompany companyName={companyName} setCompanyName={setCompanyName} city={city} setCity={setCity} />
          )}
          ...
          {step === 4 && (
            <StepUrunler urunler={urunler} setUrunler={setUrunler} kategoriler={kategoriler.filter((k) => k.enabled)} />
          )}
          {step === 5 && (
            <StepOrtaklar
              ortaklar={ortaklar}
              setOrtaklar={setOrtaklar}
              ortakCariAc={ortakCariAc}
              setOrtakCariAc={setOrtakCariAc}
            />
          )}
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: loading ? 'none' : '0 4px 20px rgba(255,87,34,0.3)',
              transition: 'all 0.2s',
            }}
          >
...
function StepWelcome({ steps }: { steps: { title: string; icon: string; desc: string }[] }) {
  return (
    <div>
      ...
      >
        <strong style={{ color: '#ff7043' }}>{BRAND_NAME}</strong> Yönetim Sistemine hoş geldiniz.
        <br />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div>
        ...
        <input
          ...
          placeholder="örn: PARSPEL Isıtma Sistemleri"
          style={inp}
          autoFocus
        />
      </div>
      <div>
        <label style={lbl}>
          Şehir / İlçe <span style={{ color: '#334155' }}>(isteğe bağlı)</span>
        </label>
        <input
          value={city}
...
function StepKasa({
  ...
  kasalar: KasaDef[];
  setKasalar: React.Dispatch<React.SetStateAction<KasaDef[]>>;
}) {
  const toggle = (i: number) =>
    setKasalar((prev) => prev.map((k, idx) => (idx === i ? { ...k, enabled: !k.enabled } : k)));
  return (
    <div>
      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.82rem', marginBottom: 14 }}>
        Kullanacağınız kasa/hesap türlerini seçin. Sonradan değiştirilebilir.
      </p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {kasalar.map((k, i) => (
...
// ── Adım 3: Kategoriler ──
function StepKategoriler({
  kategoriler,
  setKategoriler,
}: {
  kategoriler: KategoriDef[];
  setKategoriler: React.Dispatch<React.SetStateAction<KategoriDef[]>>;
}) {
  const toggle = (i: number) =>
    setKategoriler((prev) => prev.map((k, idx) => (idx === i ? { ...k, enabled: !k.enabled } : k)));
  ...
  return (
    <div>
      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.82rem', marginBottom: 14 }}>
        Hangi ürün kategorilerini kullanacaksınız? Sonradan değiştirilebilir.
      </p>
...
// ── Adım 4: Ürünler ──
function StepUrunler({
  urunler,
  setUrunler,
  kategoriler,
}: {
  urunler: UrunDef[];
  setUrunler: React.Dispatch<React.SetStateAction<UrunDef[]>>;
  kategoriler: KategoriDef[];
}) {
  ...
  return (
    <div>
      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.82rem', marginBottom: 14 }}>
        Başlangıç ürünlerinizi ekleyin. İsim boş bırakılan satırlar atlanır.
      </p>
      <div
        style={{
          display: 'flex',
...
function StepOrtaklar({
  ...
  const addWithCari = () => {
    add();
    setOrtakCariAc((prev) => [...prev, true]);
  };
  const removeWithCari = (i: number) => {
    remove(i);
    setOrtakCariAc((prev) => prev.filter((_, idx) => idx !== i));
  };
            ...
            <FormCard key={i} label={`ORTAK ${i + 1}`} onRemove={() => removeWithCari(i)} showRemove={true}>
              ...
              <div>
                <input
                  type="number"
                  value={o.share ?? ''}
                  onChange={(e) => update(i, 'share', e.target.value === '' ? undefined : parseFloat(e.target.value))}
                  placeholder="Hisse % (opsiyonel)"
                  style={inp}
                  min={0}
                  max={100}
                />
                ...
                >
                  <div
                    style={{
                      ...
                      height: 20,
                      borderRadius: 10,
                      background: (ortakCariAc[i] ?? true) ? '#ff5722' : '#334155',
                      position: 'relative',
                  ...
                  <span
                    style={{
                      fontSize: '0.82rem',
                      color: (ortakCariAc[i] ?? true) ? '#ff7043' : '#475569',
                      fontWeight: 600,
                    }}
                  >
  ...
  return (
    <div>
      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.82rem', marginBottom: 14 }}>
        İşletme ortaklarını tanımlayın. Her ortak için otomatik <strong style={{ color: '#ff7043' }}>cari hesap</strong>{' '}
        açılır — tahsilat ve emanet takibi yapılabilir.
      </p>
      {partnersContent}
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.82rem', margin: 0 }}>
        Uygulamaya giriş için parola belirleyin. En az 4 karakter.
      </p>
      <div>
