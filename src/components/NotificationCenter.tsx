...
function loadDismissed(): Set<string> {
  try {
    ...
    logger.warn('notification', 'Bildirim gizleme verisi okunamadı');
    return new Set();
  }
}
...
function saveDismissed(set: Set<string>) {
  try {
    ...
    logger.warn('notification', 'Bildirim gizleme verisi kaydedilemedi'); /* sessizce geç */
  }
}
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <div
              style={{
                ...
                fontSize: 'var(--text-base)',
                boxShadow:
                  criticalCount > 0 ? '0 4px 16px var(--color-danger-soft)' : '0 4px 16px var(--surface-overlay)',
              }}
            >
              🔔
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: 'var(--text-sm)' }}>
                Bildirimler
              </div>
          ...
          <div style={{ display: 'flex', gap: 7 }}>
            {[
              ...
              {
                count: visible.filter((n) => n.severity === 'info').length,
                label: 'Bilgi',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px' }}>
          {visible.length === 0 ? (
            ...
            >
              ...
              <div style={{ color: 'var(--color-success)', fontWeight: 700, fontSize: 'var(--text-sm)' }}>
                Her şey yolunda!
              </div>
              <div style={{ color: 'var(--text-muted)', fontSize: 'var(--text-xs)', textAlign: 'center' }}>
                Aktif uyarı yok. Tüm bildirimler okundu işaretlendi.
              </div>
              <button
