...
export function IconPicker({ value, onChange, size = 36, label }: Props) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      {label && (
        <div style={{ color: 'var(--text-muted)', fontSize: 'var(--text-sm)', fontWeight: 600, marginBottom: 5 }}>
          {label}
        </div>
      )}
      <button
      ...
      >
        {isUrl(value) ? (
          <img
            src={value}
            alt="icon"
            style={{ width: size * 0.6, height: size * 0.6, objectFit: 'contain', borderRadius: 4 }}
          />
        ) : (
          value || '📦'
        )}
              ...
              {filteredIcons.map((ic) => (
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    fontSize: '1.1rem',
                    background: value === ic ? 'var(--color-primary-soft)' : 'transparent',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                  ...
                  onMouseLeave={(e) =>
                    (e.currentTarget.style.background = value === ic ? 'var(--color-primary-soft)' : 'transparent')
                  }
                  title={ic}
                >
                  {ic}
