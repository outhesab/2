...
page.on("console", (msg) => {
  if (msg.type() === "error" || msg.text().includes("ErrorBoundary") || msg.text().includes("Error"))
...
console.log("2. Body (first 500):", bodyText.substring(0, 500));

const hasSidebar = await page.locator("aside, nav, [class*='sidebar']").first().isVisible().catch(() => false);
console.log("3. Sidebar:", hasSidebar);

// If we see the dashboard, take a screenshot
await page.screenshot({ path: "C:/Users/PARS/Desktop/clean-project/scripts/dashboard.png", fullPage: false });
